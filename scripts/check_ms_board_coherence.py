...
# US R1 keeps the formal Risk-on verdict while qualifying participation in the
# thesis. These prefixes are still semantically Risk-on; anything Risk-off/Mixed
# next to the green band remains a contradiction.
HEADLINE_ALLOWED_PREFIX = {
    "Risk-on": (
        "Risk-on —",
        "Broad risk-on —",
        "Selective risk-on —",
        "Risk-on backdrop, uneven participation —",
    ),
...
_THESIS = re.compile(r'class="v-thesis(?:\s+[^"]*)?"><span class="l-en">([^<]*)</span>')
