#!/usr/bin/env python3
"""Reproduce the Canada Opportunity Map successor fixture without ambient state.

This is an evidence recipe, not a production builder. It reuses the accepted P0B
Canada owner/action fixtures, adds only deterministic theme-action context, and
renders the current Canada Jinja template. It never reads live factordata, a
developer VM cache, collector, publish target, or ledger. The receipt binds every
repository input actually loaded by Jinja and the exact output bytes with SHA-256.
"""
...
def canada_context(
    ...
    return {
        ...
        "curve_chart": "",
        "axes_chart": "",
        "sectors": [{"ticker": f"FIXTURE-{i:02d}"} for i in range(12)],
        "actions": common_actions() if actions is None else actions,
        "theme_actions": canada_theme_actions(setups),
        "setups": setups,
        "top_setups": [],
        "stocks_health": [],
        "board_health": [],
