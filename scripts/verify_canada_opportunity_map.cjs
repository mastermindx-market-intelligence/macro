...
/*
 * Fail-closed browser receipt for the Canada Opportunity Map successor slice.
 *
 * The verifier serves one explicitly rendered Canada HTML artifact plus its checked-in
...
function usage(message) {
  ...
  process.stderr.write(
    "usage: verify_canada_opportunity_map.cjs " +
...
function themeMembershipFromFixture(filename) {
  if (!filename || !fs.existsSync(filename)) return {};
  let payload;
  try { payload = JSON.parse(fs.readFileSync(filename, "utf8")); } catch (_error) { return {}; }
  const baskets = Array.isArray(payload && payload.baskets) ? payload.baskets : [];
  const out = {};
  for (const basket of baskets) {
    if (!basket || typeof basket.id !== "string" || !Array.isArray(basket.members)) continue;
    out[basket.id] = basket.members.map((member) =>
      typeof member === "string" ? member : member && (member.symbol || member.ticker)
...
async function desktopBehavior(page, market, themeMembership = {}) {
  const prefix = market === "hk" ? "hk" : "ca";
  const version = market === "hk" ? "hk-v37" : "ca-v36";
  const hostSelector = market === "hk" ? "#standouts .nbgrid" : "#standouts .cards";
  async function manifest(label) {
    return page.evaluate(({label, market, prefix, hostSelector, themeMembership}) => {
      ...
      const themeFilter = market === "ca"
        ? document.querySelector('[data-ca-lead-kind="theme"].is-active')
        : null;
      const themeId = themeFilter ? themeFilter.getAttribute("data-ca-lead-id") : null;
      if (themeId) {
        const members = new Set(themeMembership[themeId] || []);
        expected = expected.filter((card) => members.has(String(card.getAttribute("data-ticker") || "").trim().toUpperCase()));
      }
    ...
    }, {label, market, prefix, hostSelector, themeMembership});
  }
...
async function main() {
  ...
  try {
    ...
    var opportunityMapScreenshot = null;
    if (fixtureBinding.market === "ca") {
      const section = desktopPage.locator("#ca-v36-leadership");
      if (await section.count()) {
        const bytes = await section.screenshot({animations: "disabled", caret: "hide"});
        opportunityMapScreenshot = {
          filename: "canada-opportunity-map-desktop.png",
          sha256: sha256Bytes(bytes),
        };
        if (screenshotDir) {
          const filename = path.resolve(screenshotDir, opportunityMapScreenshot.filename);
          if (path.dirname(filename) !== screenshotDir) usage("invalid opportunity-map screenshot path");
          fs.writeFileSync(filename, bytes);
          opportunityMapScreenshot.path = relativeRepoPath(repoRoot, filename);
        }
      }
    }
    const themeMembership = fixtureBinding.market === "ca"
      ? themeMembershipFromFixture(path.join(fixtureAssetsDir, "canadabasketdata", "baskets.json"))
      : {};
    var desktop = await desktopBehavior(desktopPage, fixtureBinding.market, themeMembership);
  ...
  const payload = {
    ...
    desktop,
    opportunity_map_screenshot: opportunityMapScreenshot,
