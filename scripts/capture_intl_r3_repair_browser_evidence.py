...
def capture_country(
    ...
    try:
        ...
        filename = f"proposal-r3-repair-country-{country.lower()}-{theme}-{locale}.png"
...
def capture_horizon(
    ...
    try:
        ...
        filename = f"proposal-r3-repair-{viewport}-{theme}-{locale}-horizon-{horizon}.png"
        digest, meta = screenshot(page, filename)
        assert_clean(page, telemetry, filename)
        print(f"captured {filename}", flush=True)
        return {
            "kind": "r3-repair-horizon",
            "viewport": viewport,
            "theme": theme,
            "locale": locale,
            "horizon": horizon,
...
def capture_fixed_paths(browser: Browser, reference_url: str) -> dict[str, Any]:
    ...
    try:
        ...
        filename = "proposal-r3-repair-fixed-paths-desktop-dark-en.png"
        ...
        assert_clean(page, telemetry, filename)
        if not stable or not semantic_direction_ink:
            raise AssertionError("Fixed paths changed with return horizon or used non-semantic ink")
        print(f"captured {filename}", flush=True)
        return {
            "kind": "r3-repair-fixed-paths",
            "viewport": "desktop",
            "theme": "dark",
            "locale": "en",
...
def capture_initial_zh(browser: Browser, reference_url: str) -> dict[str, Any]:
    ...
    try:
        ...
        filename = "proposal-r3-repair-mobile-dark-zh-initial-locale.png"
        digest, meta = screenshot(page, filename)
        assert_clean(page, telemetry, filename)
        print(f"captured {filename}", flush=True)
        return {
            "kind": "r3-repair-initial-locale",
            "viewport": "mobile",
            "theme": "dark",
            "locale": "zh",
...
def capture_organ_state(
    ...
    try:
        ...
        dims = dimensions(page)
        filename = f"proposal-r3-repair-organ-{state}-{theme}-en.png"
        digest, meta = screenshot(page, filename)
        assert_clean(page, telemetry, filename)
        if not all(
            (
...
def capture_stocks(
    ...
    try:
        ...
        filename = f"stocks-r3-repair-{viewport}-{theme}-{locale}.png"
        digest, meta = screenshot(page, filename)
        assert_clean(page, telemetry, filename)
        print(f"captured {filename}", flush=True)
        return {
            "kind": "stocks-repair-preservation",
            "viewport": viewport,
            "theme": theme,
            "locale": locale,
            "settled": settled,
...
def semantic_checks(browser: Browser, reference_url: str) -> dict[str, Any]:
    ...
    try:
        ...
        direction_en = page.locator(".path-card .sparkline").evaluate_all(
            "nodes => nodes.map(node => getComputedStyle(node.querySelector('polyline')).stroke)"
        )
        tokens = [
            "--ink-up",
            "--ink-down",
...
def capture() -> None:
    ...
    direct_initial = next(
        row for row in captures if row["kind"] == "r3-repair-initial-locale"
    )
    fixed_paths = next(
        row for row in captures if row["kind"] == "r3-repair-fixed-paths"
    )
    organ_rows = [row for row in captures if row["kind"] == "r3-repair-organ-state"]
    stocks_rows = [
        row for row in captures if row["kind"] == "stocks-repair-preservation"
    ]
    all_console_errors = [
    ...
    proof = {
        "schema": "mastermind.intl_r3_browser_evidence.v3",
