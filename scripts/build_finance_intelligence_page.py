"""Render the data-free Finance Intelligence dossier shell.

Sector and slice facts are intentionally *not* baked into ``site/``.  The page
loads its current, verified read-model projection only from the same-origin
foundation route at runtime.  That keeps a registered preview useful without
turning a static render or the public git tree into a private distribution
channel.

Per Chairman directive 2026-09-24 relayed from Astra CEO: Finance does NOT
build base layers — no private store, no publish lane, no serving route.
This module renders the static shell and the paired client assets only.

Usage::

    python -m scripts.build_finance_intelligence_page
"""
...
def render_shell(root: Path) -> Path:
    ...
    page = site / "finance_intelligence.html"
