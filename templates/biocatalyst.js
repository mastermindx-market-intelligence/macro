(function () {
  ...
  // WMN detail is generation-bound.  Reuse the existing Trial Intelligence
  // renderer without issuing the latest-record TRIAL_API request that
  // selectTrial() owns.  This bridge is intentionally narrow: the WMN client
  // may present a trial already validated by its own same-generation detail
  // route, but it cannot read or mutate the Trial workspace data plane.
  function showGenerationBoundTrialDetail(detail, trigger) {
    if (!validTrial(detail)) throw new Error('Invalid generation-bound trial detail contract');
    state.selectedId = nctOf(detail); state.selectedKey = ''; state.selected = detail; state.detail = detail; state.evidenceCell = null;
    abort('detailController'); state.detailToken += 1;
    trigger = trigger && document.contains(trigger) ? trigger : null;
    openInspector(window.matchMedia('(max-width: 1120px)').matches, trigger);
    showDetail(detail, null, null);
  }
  window.BioCatalystTrialWorkspace = Object.freeze({
    showGenerationBoundTrialDetail: showGenerationBoundTrialDetail
  });
...
(function () {
  ...
  function humanize(value){var s=clean(value).replace(/_/g,' ').toLowerCase();return s?s.charAt(0).toUpperCase()+s.slice(1):tr('Unavailable','暂不可用')}
  function abort(name){if(state[name])state[name].abort();state[name]=null}
  function withAuth(headers){headers=headers||{};if(!(window.MDXAuth&&window.MDXAuth.client))return Promise.resolve(headers);return window.MDXAuth.client().then(function(client){return client.auth.getSession()}).then(function(result){var token=result&&result.data&&result.data.session&&result.data.session.access_token;if(token)headers.Authorization='Bearer '+token;return headers}).catch(function(){return headers})}
  function fetchJson(url,signal){return withAuth({Accept:'application/json'}).then(function(headers){return fetch(url,{headers:headers,credentials:'same-origin',cache:'no-store',signal:signal})}).then(function(response){if(response.status===401||response.status===402||response.status===403){var locked=new Error('FULL_ACCESS_REQUIRED');locked.status=response.status;throw locked}if(response.status===404){var missing=new Error('NOT_FOUND');missing.status=404;throw missing}if(response.status===409){var reload=new Error('RELOAD_REQUIRED');reload.status=409;throw reload}if(!response.ok){var unavailable=new Error('HTTP_'+response.status);unavailable.status=response.status;throw unavailable}return response.json()})}
  function validWmnAuthority(authority){return !!authority&&authority.classification==='research_priority_only'&&authority.trade_origination === false&&authority.changes_availability === false&&authority.position_sizing === false&&authority.prophet_admission === false}
  function validEstimate(slot){return !!slot&&typeof slot==='object'&&slot.state==='NOT_ESTIMABLE'&&slot.value===null&&!!clean(slot.reason_code)&&Array.isArray(slot.evidence_refs)}
  function wmnIdentity(row){var key=valueAt(row,'row_key')||{};return clean(valueAt(row,'event_fact_ref'))+'|'+clean(valueAt(key,'issuer_id'))}
  function validWmnRow(row){var priority=valueAt(row,'research_priority'),timing=valueAt(row,'timing'),issuer=valueAt(row,'issuer'),links=valueAt(row,'links');if(!row||typeof row!=='object'||!clean(row.event_fact_ref)||!clean(row.event_family))return false;if(!priority||priority.disposition!=='SELECTED'||!WMN_LANES[clean(priority.lane)]||clean(priority.method_id)!==WMN_METHOD_ID||!validWmnAuthority(priority.authority))return false;if(!timing||typeof timing!=='object'||!issuer||typeof issuer!=='object')return false;if(!validEstimate(row.probability)||!validEstimate(row.materiality)||!validEstimate(row.historical_response)||!validEstimate(row.incorporation))return false;return !!links&&typeof links==='object'&&Array.isArray(links.stock_research)}
  function validEnvelope(payload){var pagination=valueAt(payload,'pagination'),rows=valueAt(payload,'rows');return !!payload&&payload.contract_id==='biocatalyst_what_matters_next.v1'&&payload.schema_version==='1.0.0'&&!!clean(payload.generation_id)&&clean(payload.method_id)===WMN_METHOD_ID&&validWmnAuthority(payload.authority)&&!!payload.coverage&&typeof payload.coverage==='object'&&Array.isArray(rows)&&rows.every(validWmnRow)&&!!pagination&&Number.isSafeInteger(pagination.total)&&Number.isSafeInteger(pagination.limit)&&pagination.limit===WMN_PAGE_LIMIT&&(pagination.next_cursor===null||typeof pagination.next_cursor==='string')}
  function validGenerationBoundTrial(trial){return trial===null||(!!trial&&typeof trial==='object'&&/^NCT\d{8}$/.test(clean(trial.nct_id))&&!!(clean(trial.title)||clean(trial.brief_title)))}
  function validDetail(payload){return !!payload&&payload.contract_id==='biocatalyst_wmn_detail.v1'&&payload.schema_version==='1.0.0'&&clean(payload.generation_id)===state.generation&&validWmnAuthority(payload.authority)&&validWmnRow(payload.event)&&validGenerationBoundTrial(payload.trial)&&Array.isArray(payload.related_events)&&payload.related_events.every(validWmnRow)}
  function laneLabel(lane){var labels={ACT_NOW:['Review soon','近期核查'],RECONCILE:['Needs review','需要核实'],RESEARCH_NEXT:['Research next','后续研究'],MONITOR:['Monitor','持续关注']},pair=labels[clean(lane)]||[humanize(lane),humanize(lane)];return tr(pair[0],pair[1])}
  function laneExplanation(lane){var copy={ACT_NOW:['The scheduled window is entirely within the next 7 calendar days. Check its evidence.','计划窗口完全位于未来7个自然日内，请核查相关证据。'],RECONCILE:['Resolve the highlighted source, timing or identity gap before relying on this record.','请先核实标出的来源、时间或身份信息缺口。'],RESEARCH_NEXT:['This window overlaps the next 90 calendar days; its full timing precision remains shown.','该窗口与未来90个自然日重叠，页面保留其原始时间精度。'],MONITOR:['A later schedule or resolved event history; this is not a return forecast.','这是较远期计划或已核实的事件历史，并非收益预测。']},pair=copy[clean(lane)]||[humanize(lane),humanize(lane)];return tr(pair[0],pair[1])}
  function issuerLabel(row){var issuer=valueAt(row,'issuer')||{},securities=arr(valueAt(issuer,'securities'));if(clean(issuer.state)==='resolved'&&securities.length){var symbols=securities.map(function(s){return clean(valueAt(s,'display_symbol'))}).filter(Boolean);if(symbols.length)return symbols.join(' · ');if(clean(issuer.issuer_id))return clean(issuer.issuer_id)}if(clean(issuer.state)==='ambiguous')return tr('Ambiguous issuer','发行人不明确');return tr('Issuer unresolved','发行人未解析')}
  function timingLabel(row){var timing=valueAt(row,'timing')||{},lower=clean(timing.lower_date),upper=clean(timing.upper_date);if(lower&&upper)return lower===upper?lower:lower+' → '+upper;return lower||upper||tr('Timing unresolved','时间未确定')}
  function timingSource(row){return humanize(valueAt(valueAt(row,'timing'),'source_class'))}
  function missingReasons(row){var missing=valueAt(row,'missingness')||{},reasons=[];Object.keys(missing).forEach(function(key){arr(missing[key]).forEach(function(reason){reason=clean(reason);if(reason&&reasons.indexOf(reason)<0)reasons.push(reason)})});return reasons}
  function estimateChip(name,slot){var chip=el('span','bci-wmn-estimate');text(chip,name+': '+(slot&&slot.state==='NOT_ESTIMABLE'?'NOT_ESTIMABLE':tr('Unavailable','暂不可用')));if(slot&&clean(slot.reason_code))chip.title=humanize(slot.reason_code);return chip}
  function stockLinks(row){return arr(valueAt(valueAt(row,'links'),'stock_research'))}
  function makeWmnRow(row,index){var key=wmnIdentity(row),card=el('article','bci-wmn-card'+(key===state.selectedKey?' is-selected':''));card.setAttribute('role','option');card.setAttribute('aria-selected',key===state.selectedKey?'true':'false');card.setAttribute('data-wmn-key',key);card.tabIndex=index===0?0:-1;var main=el('div','bci-wmn-main'),top=el('div','bci-wmn-topline');top.appendChild(el('span','bci-wmn-symbol',issuerLabel(row)));top.appendChild(el('span','bci-wmn-family',humanize(row.event_family)));top.appendChild(el('span','bci-wmn-lane',laneLabel(valueAt(row.research_priority,'lane'))));main.appendChild(top);main.appendChild(el('div','bci-wmn-title',humanize(row.event_family)+' · '+timingLabel(row)));main.appendChild(el('div','bci-wmn-why',laneExplanation(valueAt(row.research_priority,'lane'))));var estimates=el('div','bci-wmn-estimates');estimates.appendChild(estimateChip(tr('Probability','概率'),row.probability));estimates.appendChild(estimateChip(tr('Materiality','重要性'),row.materiality));estimates.appendChild(estimateChip(tr('History','历史反应'),row.historical_response));estimates.appendChild(estimateChip(tr('Incorporation','市场计入'),row.incorporation));main.appendChild(estimates);var missing=missingReasons(row);if(missing.length)main.appendChild(el('div','bci-wmn-missing',tr('Open gaps: ','待补缺口：')+missing.slice(0,4).map(humanize).join(' · ')));var links=stockLinks(row);if(links.length){var wrap=el('div','bci-wmn-stock-links');links.forEach(function(item){var href=clean(valueAt(item,'href'));if(!/^stock\.html\?ticker=/.test(href))return;var a=el('a','',tr('Open stock research','打开股票研究'));a.href=href;a.addEventListener('click',function(event){event.stopPropagation()});wrap.appendChild(a)});main.appendChild(wrap)}card.appendChild(main);var timing=el('div','bci-wmn-timing');timing.appendChild(el('strong','',timingLabel(row)));timing.appendChild(el('span','',timingSource(row)));card.appendChild(timing);card.addEventListener('click',function(){selectWhatMattersNextRow(row,card)});card.addEventListener('keydown',function(event){if(event.key!=='Enter'&&event.key!==' ')return;event.preventDefault();selectWhatMattersNextRow(row,card)});return card}
  function renderQueue(){clear(queue);queue.setAttribute('aria-busy','false');if(!state.rows.length){var copy=state.view==='reconcile'?tr('No unresolved research items in this cut.','此数据切片中没有待核对研究项。'):state.view==='history'?tr('No resolved historical items match this search.','没有符合此搜索的已解决历史事件。'):tr('No research-priority events match this horizon and search.','没有符合当前窗口和搜索条件的研究优先事件。');queue.appendChild(el('div','bci-wmn-detail-empty',copy))}else state.rows.forEach(function(row,index){queue.appendChild(makeWmnRow(row,index))});loadMoreNode.hidden=!state.nextCursor;loadMoreNode.disabled=false}
  function renderCoverage(payload){var coverage=valueAt(payload,'coverage')||{};clear(coverageNode);[[valueAt(coverage,'source_event_count'),tr('source events','来源事件')],[valueAt(coverage,'issuer_event_count'),tr('issuer exposures','发行人敞口')],[valueAt(coverage,'security_count'),tr('securities','证券')],[valueAt(coverage,'unresolved_event_count'),tr('unresolved','未解析')]].forEach(function(pair){if(Number.isSafeInteger(pair[0]))coverageNode.appendChild(el('span','',pair[0]+' '+pair[1]))});var ports=arr(valueAt(coverage,'missing_owner_ports'));if(ports.length)coverageNode.appendChild(el('span','',tr('Missing owners: ','缺失数据所有者：')+ports.length))}
  function renderStatus(payload){var boardState=clean(valueAt(payload,'state')),total=valueAt(valueAt(payload,'pagination'),'total');root.dataset.state=boardState||'ready';var copy=boardState==='partial'?tr('Partial decision-intelligence cut — gaps are kept visible','部分决策智能数据——缺口保持可见'):boardState==='stale'?tr('Stale source cut — research with caution','来源数据已陈旧——研究时请谨慎'):boardState==='empty'?tr('No matching research priorities','没有匹配的研究优先项'):tr('Current research-priority cut','当前研究优先级数据');text(statusNode,copy+(Number.isSafeInteger(total)?' · '+total:''));var cutoff=clean(valueAt(payload,'evaluation_cutoff'));text(asOfNode,cutoff?tr('Evaluation cutoff ','评估截止 ')+cutoff.replace('T',' ').replace('Z',' UTC'):'');renderCoverage(payload)}
  function section(title){var box=el('section','bci-wmn-detail-section');box.appendChild(el('h4','',title));return box}
  function addPair(box,label,value){var p=el('p');p.appendChild(el('strong','',label+': '));p.appendChild(document.createTextNode(value));box.appendChild(p)}
  function showWmnDetail(payload){var row=payload.event,priority=row.research_priority;clear(detail);detail.appendChild(el('p','bci-kicker',tr('Selected research event','已选研究事件')));detail.appendChild(el('h3','',issuerLabel(row)+' · '+humanize(row.event_family)));var timing=section(tr('Timing','时间'));addPair(timing,tr('Window','窗口'),timingLabel(row));addPair(timing,tr('Source class','来源类型'),timingSource(row));addPair(timing,tr('Occurrence','发生状态'),humanize(row.occurrence));detail.appendChild(timing);var research=section(tr('Research priority','研究优先级'));addPair(research,tr('Lane','通道'),laneLabel(priority.lane));addPair(research,tr('Why now','为什么现在'),laneExplanation(priority.lane));addPair(research,tr('Rule reason','规则原因'),humanize(priority.primary_reason));if(arr(priority.gap_reasons).length)addPair(research,tr('Open gaps','待补缺口'),arr(priority.gap_reasons).map(humanize).join(' · '));detail.appendChild(research);var estimates=section(tr('Uncertainty','不确定性'));['probability','materiality','historical_response','incorporation'].forEach(function(name){var slot=row[name];addPair(estimates,humanize(name),slot.state+' — '+humanize(slot.reason_code))});detail.appendChild(estimates);var exposure=section(tr('Exposure and evidence','敞口与证据'));addPair(exposure,tr('Economic exposure','经济敞口'),humanize(row.economic_exposure_state));var missing=missingReasons(row);addPair(exposure,tr('Missing intelligence','缺失智能'),missing.length?missing.map(humanize).join(' · '):tr('None declared','未声明缺口'));addPair(exposure,tr('Evidence objects','证据对象'),String(arr(row.evidence).length));detail.appendChild(exposure);var links=stockLinks(row);if(links.length){var nav=section(tr('Research continuation','继续研究'));links.forEach(function(item){var href=clean(item.href);if(!/^stock\.html\?ticker=/.test(href))return;var a=el('a','bci-wmn-trial-link',tr('Open stock research','打开股票研究'));a.href=href;nav.appendChild(a)});detail.appendChild(nav)}if(payload.related_events.length){var related=section(tr('Related issuer events','相关发行人事件'));addPair(related,tr('Same issuer in this generation','同一发行人在本代数据中'),String(payload.related_events.length));detail.appendChild(related)}detail.appendChild(el('div','bci-wmn-authority',tr('Research triage only — not a trade signal. This view cannot originate trades, change Availability, size positions or admit Prophet.','仅用于研究分流——不是交易信号。此视图不能发起交易、改变 Availability、决定仓位或接入 Prophet。')))}
  function showDetailMessage(title,body){clear(detail);detail.appendChild(el('p','bci-kicker',tr('Selected research event','已选研究事件')));detail.appendChild(el('h3','',title));detail.appendChild(el('p','bci-wmn-detail-empty',body))}
  function selectWhatMattersNextRow(row,trigger){state.selectedKey=wmnIdentity(row);Array.prototype.slice.call(queue.querySelectorAll('.bci-wmn-card')).forEach(function(card){var selected=card.getAttribute('data-wmn-key')===state.selectedKey;card.classList.toggle('is-selected',selected);card.setAttribute('aria-selected',selected?'true':'false')});showDetailMessage(tr('Loading event dossier…','正在加载事件档案…'),tr('Resolving the selected event inside the same immutable generation.','正在同一不可变数据代中解析所选事件。'));if(trigger&&typeof trigger.focus==='function')trigger.focus({preventScroll:true});abort('detailController');var controller=new AbortController(),token=state.detailToken+1;state.detailController=controller;state.detailToken=token;var params=new URLSearchParams();params.set('generation_id',state.generation);params.set('event_fact_ref',row.event_fact_ref);var issuerId=clean(valueAt(valueAt(row,'row_key'),'issuer_id'));if(issuerId)params.set('issuer_id',issuerId);fetchJson(WMN_DETAIL_API+'?'+params.toString(),controller.signal).then(function(payload){if(token!==state.detailToken)return;if(!validDetail(payload))throw new Error('INVALID_DETAIL_CONTRACT');showWmnDetail(payload);if(payload.trial!==null){var trialWorkspace=window.BioCatalystTrialWorkspace;if(!trialWorkspace||typeof trialWorkspace.showGenerationBoundTrialDetail!=='function')throw new Error('TRIAL_WORKSPACE_BRIDGE_UNAVAILABLE');trialWorkspace.showGenerationBoundTrialDetail(payload.trial,trigger)}else detail.focus({preventScroll:true})}).catch(function(error){if(error&&error.name==='AbortError')return;if(error&&error.status===409)showDetailMessage(tr('Research cut changed','研究数据已更新'),tr('Refresh What Matters Next to reopen this event in the current generation.','请刷新“下一步看什么”，以当前数据代重新打开此事件。'));else if(error&&error.status===404)showDetailMessage(tr('Event not in this generation','此数据代中没有该事件'),tr('The event is no longer present in the exact generation you selected.','该事件已不在你所选的精确数据代中。'));else showDetailMessage(tr('Dossier unavailable','档案暂不可用'),tr('No replacement facts are inferred. Refresh the research queue or use Trial Intelligence for source-level evidence.','不会推断替代事实。请刷新研究队列，或使用试验智能查看来源级证据。'))}).finally(function(){if(state.detailController===controller)state.detailController=null})}
  function queryUrl(cursor){var params=new URLSearchParams();params.set('view',state.view);params.set('limit',String(WMN_PAGE_LIMIT));if(state.view==='upcoming')params.set('horizon_days',state.horizon);if(state.q)params.set('q',state.q);if(state.view==='upcoming'&&state.lane)params.set('lane',state.lane);if(cursor)params.set('cursor',cursor);return WMN_API+'?'+params.toString()}
  function setLoading(loading){state.loading=loading;refreshNode.disabled=loading;loadMoreNode.disabled=loading;queue.setAttribute('aria-busy',loading?'true':'false')}
  function loadQueue(append){if(state.loading)return;var cursor=append?state.nextCursor:'';if(append&&!cursor)return;abort('listController');var controller=new AbortController(),token=state.listToken+1;state.listController=controller;state.listToken=token;setLoading(true);if(!append){state.rows=[];state.nextCursor='';state.total=0;state.generation='';state.selectedKey='';showDetailMessage(tr('Choose an event','选择一个事件'),tr('Open a row to inspect timing, issuer exposure, research priority and missing intelligence.','打开一行，查看时间、发行人敞口、研究优先级和缺失智能。'));clear(queue);queue.appendChild(el('div','bci-wmn-detail-empty',tr('Loading research priorities…','正在加载研究优先项…')))}fetchJson(queryUrl(cursor),controller.signal).then(function(payload){if(token!==state.listToken)return;if(!validEnvelope(payload))throw new Error('INVALID_WMN_CONTRACT');if(append&&state.generation&&clean(payload.generation_id)!==state.generation){state.loading=false;window.setTimeout(function(){loadQueue(false)},0);return}var incoming=payload.rows,seen={};state.rows.forEach(function(row){seen[wmnIdentity(row)]=true});incoming.forEach(function(row){var key=wmnIdentity(row);if(seen[key])throw new Error('DUPLICATE_WMN_ROW');seen[key]=true});state.rows=append?state.rows.concat(incoming):incoming.slice();state.generation=clean(payload.generation_id);state.total=payload.pagination.total;state.nextCursor=clean(payload.pagination.next_cursor);renderStatus(payload);renderQueue()}).catch(function(error){if(error&&error.name==='AbortError')return;if(append&&error&&error.status===409){state.loading=false;window.setTimeout(function(){loadQueue(false)},0);return}root.dataset.state=error&&(error.status===401||error.status===402||error.status===403)?'locked':'error';clear(queue);queue.appendChild(el('div','bci-wmn-detail-empty',root.dataset.state==='locked'?tr('Full access is required before biotechnology research priorities can be shown.','需要完整访问权限才能显示生物科技研究优先项。'):tr('What Matters Next is temporarily unavailable. No research priority is inferred.','“下一步看什么”暂时不可用。不会推断任何研究优先级。')));text(statusNode,root.dataset.state==='locked'?tr('Full access required','需要完整访问权限'):tr('Decision-intelligence cut unavailable','决策智能数据暂不可用'));text(asOfNode,'');clear(coverageNode);loadMoreNode.hidden=true}).finally(function(){if(state.listController===controller)state.listController=null;setLoading(false)})}
  function localizeControls(){Array.prototype.slice.call(viewNode.options).concat(Array.prototype.slice.call(laneNode.options)).forEach(function(option){var label=option.getAttribute(lang()==='zh'?'data-label-zh':'data-label-en');if(label)text(option,label)});searchNode.placeholder=searchNode.getAttribute(lang()==='zh'?'data-placeholder-zh':'data-placeholder-en')||searchNode.placeholder}
  function syncControls(){horizonButtons.forEach(function(button){var active=button.getAttribute('data-horizon')===state.horizon;button.classList.toggle('is-active',active);button.setAttribute('aria-pressed',active?'true':'false');button.disabled=state.view!=='upcoming'});laneNode.disabled=state.view!=='upcoming';localizeControls()}
  function bind(){var debounce=0;searchNode.addEventListener('input',function(){window.clearTimeout(debounce);debounce=window.setTimeout(function(){state.q=clean(searchNode.value).slice(0,100);loadQueue(false)},260)});viewNode.addEventListener('change',function(){state.view=WMN_VIEWS[viewNode.value]?viewNode.value:'upcoming';if(state.view!=='upcoming')state.lane='';laneNode.value=state.lane;syncControls();loadQueue(false)});laneNode.addEventListener('change',function(){state.lane=clean(laneNode.value);loadQueue(false)});horizonButtons.forEach(function(button){button.addEventListener('click',function(){var value=clean(button.getAttribute('data-horizon'));if(!WMN_HORIZONS[value]||state.view!=='upcoming'||state.horizon===value)return;state.horizon=value;syncControls();loadQueue(false)})});refreshNode.addEventListener('click',function(){loadQueue(false)});loadMoreNode.addEventListener('click',function(){loadQueue(true)});document.addEventListener('langchange',function(){syncControls();renderQueue();if(state.selectedKey){var selected=state.rows.filter(function(row){return wmnIdentity(row)===state.selectedKey})[0];if(selected)showDetailMessage(tr('Event selected','已选择事件'),tr('Open the row again to refresh the localized dossier without changing the research cut.','再次打开该行即可刷新本地化档案，研究数据切片不会改变。'))}})}
  function init(){root=document.getElementById('bci-wmn');if(!root)return;queue=document.getElementById('bci-wmn-queue');detail=document.getElementById('bci-wmn-detail');statusNode=document.getElementById('bci-wmn-status');asOfNode=document.getElementById('bci-wmn-asof');coverageNode=document.getElementById('bci-wmn-coverage');searchNode=document.getElementById('bci-wmn-search');viewNode=document.getElementById('bci-wmn-view');laneNode=document.getElementById('bci-wmn-lane');refreshNode=document.getElementById('bci-wmn-refresh');loadMoreNode=document.getElementById('bci-wmn-load-more');horizonButtons=Array.prototype.slice.call(root.querySelectorAll('[data-horizon]'));syncControls();bind();loadQueue(false)}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
})();
