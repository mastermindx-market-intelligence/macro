function _plvMode(d, et){
  ...
  /* A closed market does not repair an incomplete final observation.
     Preserve the existing threshold and keep wall-clock ageing separate. */
  ...
  var incomplete=isFinite(ev)&&ev>0&&(sum/ev)>=0.5;
  ...
  if(et.min>PLV_SHUT){
    if(d.status!=='dark'&&incomplete) return {mode:'dark', why:'coverage'};
  ...
  if(incomplete) return {mode:'dark', why:'quotes'};
