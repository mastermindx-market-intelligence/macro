# Prophet US — Fable Meta-CEO Effective Master Plan R6

> **R6 execution responsibility amendment, September 23, 2026.** The complete R5 product/science plan is retained below with chapter21 and chapter26 superseded for the current Fable Meta-CEO assignment. Fable is accountable for end-to-end execution; delegated work uses the existing external fabric only, never built-in/native subagents. No mandatory Astra return remains. The research proposals still require their scientific evidence; no source grant, trade permission, dispatched worker or production result is inferred. See `../SUPERSESSION_MAP.md`, `../DECISION_RESOLUTION_PLAYBOOK.md` and `../HANDOFF_STATUS.json`. R5 itself is preserved byte-identically in `../baseline_r5/`.


**Effective version:** R6 / 2026-09-23; product/science baseline R5 retained  
...
**Current operation:** `prophet-us-fable-meta-ceo-20260923-001`; historical R5 operation `prophet-us-master-plan-r5-20260923-sol-001`  
**Current handoff checkpoint:** Macro #6805 issue comment `5793406610`. Historical R5 synthesis: comment `5791272310`; its mandatory Astra resume is superseded.  
**R6 preparation procedure pin:** `Mastermind@bf764f494b9cd0ecede6234bb472c3344c8e77cc`, protected master, compatible Skillpack 1.0.1 / bootstrap-major 1. Re-pin at pickup; the historical R5 pin is retained in the exact baseline archive.  
**R6 preparation snapshot:** `macro@fc5c1fc49e4f552d8e3e55b5528a6d280d7415e8`. Historical R5 planning snapshot: `macro@c9978765aa0085eba0213430284cb4b41065d4e2`. Neither is an assertion about the deployed release.  
**Inputs:** four hash-verified R1–R4 dossiers, reproduced in `../baseline_r5/inputs/`; their 25,292 words are supporting analysis, not additional live authority.  
...
## 21. Fable Meta-CEO and external-fabric execution

**The current Chairman delegation makes Fable the end-to-end Meta-CEO for this scoped Prophet US program. No required return to Astra remains.** Fable owns product direction, hard scientific and architecture judgment, decisions D01–D12, independent-review consumption, adoption, release adjudication and final program acceptance. Existing technical, data, source-custody and runtime owners remain intact. The detailed operative procedure is `../FABLE_META_CEO_EXECUTION_HANDOFF.md`.

### 21.1 Research ownership

Fable owns all Q01–Q24 and their final scientific decisions. Eligible external specialists perform bounded source work, experiment implementation, replication and methodological review. Fable directly handles the hardest residual reasoning when qualified specialists cannot resolve it, and returns routine work to the external fabric once the issue is settled. A separate Astra Pro session may be an optional consultation but never a required approval, dependency or stop condition.

Every research task has exact source/data/model vintages, a question, controls, falsifier, protected-outcome boundary and consumer. A negative or inconclusive result remains honest. Model, policy, entry and public-claim promotion keep their own registered evidence gates. No wording in this plan licenses real-money trades.

### 21.2 External-only delegation

Use the **existing external subagent fabric only**. Built-in/native Opus/Sonnet/Haiku agents, Agent Teams, internal-agent fallback and delegating through a wrapper that silently spawns native children are prohibited. All descendants inherit the restriction. Normal deterministic tools and Fable's own principal reasoning are not subagents.

The current Capacity/Model Router chooses the least-scarce capable eligible external worker. Verify actual route, budget, custody and return evidence before effect. Use the installed accepted ingress/schema; do not invent provider APIs, accounts, credentials or extra parameters. An architecture document is not proof that a tool or provider is live. Missing external capacity holds that child lane, not permission for an internal fallback or another dispatcher.

### 21.3 Fable's operating duty

Prioritize the next real capability; resolve interface and scientific decisions; form bounded packets; reserve independent review; consume exact-head results; handle exceptional hard problems; and advance the next ready dependency. Do not personally write every adapter or poll unchanged CI. Read the `../DECISION_RESOLUTION_PLAYBOOK.md` for the twelve closed decision procedures and `../FIRST_WAVE_RUNBOOK.md` for initial work.

A direct Fable code/research exception names the exact difficult question, failed distinct specialist approaches or principal-only requirement, source scope, allowed effect, stop condition and independent reviewer. It must not become a blanket migration of routine labor to Fable.

### 21.4 Work packet and return

All B00–B28 and Q01–Q24 have individual `../work_cards/` briefs, supplied through the existing worker-brief/admission owner. Each actual child binds current source and runtime identity, permitted paths, accepted interfaces, research boundaries, negative tests, real proof, budget, external-only placement and exact return. This is one program-level delegation, not one all-repository mutating child. Do not send the whole vision as an unbounded implementation commission.

Delivery, ACK, START/RUNNING, source commit, independent review, merge, deployment, normal refresh and acceptance are distinct. Existing started or effect-unknown work remains on its original carrier. Fable can close a scoped program HOLD-FOR-SOL after its real evidence gates clear; the former principal's name is not a reason to wait forever. It cannot bypass source ownership or replace independent review.

Chairman escalation is limited to genuinely reserved human/credential/legal/new-spend/material-scope or real-money decisions. Routine design, methodology, debugging, sequencing and in-scope acceptance are Fable's responsibility, with no mandatory Astra stage.
...
## 26. Immediate execution and completion boundary

The next interaction is deliberate execution delivery of the R6 archive and `../FABLE_START_PROMPT.md` to one eligible Fable session. At preparation, no Fable ACK/START or child dispatch has occurred. Verify `../MANIFEST.json` using `python ../verify_handoff.py`, read current protected procedure, acknowledge the actual assignment, reconcile previous operation/writer/reviewer state and establish the real admitted external-fabric return path.

Fable reviews and adopts this source plan through B00 and the existing owner, publishes the included Agent OS delegation record under current source gates, and starts the first genuinely ready unit. Existing #7581 validity, #7180/#7572 delivery and source-bound Earnings/D5/GMI evidence are the first execution targets. No repeat broad architecture audit and no obligatory Astra approval are required.

Use branch-level activation in `ACTIVATION_GATES.json`. Historical board-row analysis does not globally wait for live B4 or additional anchor species; episode and prospective B4 claims retain their own necessary prerequisites. Research-to-watch can show honest unavailable entry. Offline model research need not wait for UI, but live ranking and recommendations require their separate acceptance.

Continue through all 29 build units and all 24 research dispositions, not only the first PRs. Final acceptance requires the advertised three-sleeve user jobs, exact source/model/policy truth, actual private/entitled browser and saved-state proof, honest evaluation, normal refresh, monitoring and rollback. Unproven hypotheses stay unpromoted; missing promised capability stays an obligation.

The program is not complete at this handoff. The prepared artifacts and canonical record establish a recoverable execution assignment, not a running daemon or a deployed product. Fable owns the subsequent accountable decisions and completion under the Chairman's scoped delegation.

## References and evidence scope

### Internal inputs and fresh sources

- **R1:** `../baseline_r5/inputs/PROPHET_US_RESEARCH_R1.md`, SHA-256 `96d0ab34f5db94537fe60b493af166b0c50ca8eac2b89641703d9bb290166465`.
- **R2:** `../baseline_r5/inputs/PROPHET_US_MEASUREMENT_R2.md`, SHA-256 `32bf1ddba37aa269469d7fb641dcf862ff4ea0aad3a567acdffa857cb26de19e`.
- **R3:** `../baseline_r5/inputs/PROPHET_US_EARLY_LEADERSHIP_R3.md`, SHA-256 `4ad7b39e8b4916029755f863d3b56ba0dc30d75ff46d95b031ceb24cb8946ff5`.
- **R4:** `../baseline_r5/inputs/PROPHET_US_EARNINGS_CYCLE_R4.md`, SHA-256 `e9464a12b8faba1e61b2d8ed6cb3d508f7c99efd41a6e3b4ee715a849fd593d2`.
