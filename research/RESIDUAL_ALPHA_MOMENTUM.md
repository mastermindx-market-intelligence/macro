...
**REQUALIFICATION HOLD (2026-09-24, issue #7954):** the historical Phase-0/Phase-2 *portfolio* headline numbers below are development evidence, not current acceptance evidence, until rerun through the corrected harness. The legacy monthly sleeve could carry positions that a rebalance set to zero, clipped genuine daily outcomes at ±50%, and silently omitted held-name missing returns. The IC path also let future-label availability shrink the predictor cross-section before sector neutralization. Those mechanisms are now corrected in `scripts/residual_alpha_phase0.py`; the old report remains historical provenance and must not be silently overwritten. Rank-IC conclusions are not automatically reversed by this hold, but any claim that depends on the old long/short Sharpe/cumulative return or future-conditioned sector normalization requires requalification. The production `engine/residual_alpha.py` score is unchanged by this research hold.

**STAGE-2 DATA QUALIFICATION HOLD (2026-09-24, issue #7954):** PIT membership is not PIT price coverage. The real-data Alpha Leadership development run had median feature support of only 65.24% of eligible PIT members. A follow-up host audit found Yahoo/dead-price support for only 11.73% of membership tickers whose last tenure ended before 2010 and 12.50% for 2010–2019 exits, versus 94.42% for names with an open/active tenure. The 63-session conditional-continuity result is therefore method/development evidence only; 21/126 secondaries are held pending price/identity repair. See `reports/alpha-leadership-stage2-data-qualification.json`. This hold does not change the production residual-alpha ranker or imply the continuity effect is proven absent.
