# TTI R1-B v4 preregistration — exhaustion/reclaim versus continuation

**Status: v4 candidate preregistration; NOT REGISTERED; NOT RUN.**

V4 supersedes the **unregistered/unrun** v3 freeze at `1518ef7dabc74428bd79e7724f080226381b6fe4` after a third pre-outcome adversarial review found two machine-config identity ambiguities: a leftover global `first_event_per_symbol_day=true` contradicted the per-selector firing law, and a singular candidate-low LOD anchor coexisted with the declared two-anchor diagnostics. `research/species/tti_r1b/V3_ADVERSARIAL_REVIEW.md` records the correction. V1–v3 remain immutable DO_NOT_RUN audit artifacts. No market-state threshold, selector, horizon, cost cell, population, or outcome rule changed.

V3 had already superseded v2 after adding one full five-minute processing latency; v2 had superseded v1 after correcting future-label control conditioning, selector identity and low-anchor bookkeeping. Those corrections remain in v4 unchanged.
...
Let `Lpre(t)` be the minimum low of all completed **positive-volume** RTH bars strictly before the candidate bar. Zero-volume aggregates remain visible as missing-participation diagnostics but do not establish a session low. The candidate bar is a **fresh low** only when its low `L(t)` is strictly below `Lpre(t)`. If no lawful prior positive-volume price observation exists, the candidate is unavailable.

The base episode additionally requires:

1. `(previous_RTH_close - L(t)) / ATR20 >= 0.50`.
2. The trailing three completed five-minute bars including the candidate are all positive-volume price observations and show `(open_of_oldest_trailing_bar - L(t)) / ATR20 >= 0.20`.
...
`BASE_FRESH_LOW` fires at candidate decision time. **Execution latency is one full five-minute bar.** The bar starting at decision time is treated as the processing/latency interval; the price-reference entry is the open five minutes later. It remains a price reference rather than a guaranteed fill.
...
Confirmed-family decision time is the confirming bar completion. **Execution latency is then one additional full five-minute bar**: the bar starting at confirmation time is the processing interval and the price-reference entry is the open five minutes later. Confirmation delay and processing latency are stored separately. No event is backpainted onto the candidate extreme.

**Selector identity is independent and explicit.** There is no global first-event-per-symbol/day gate. Each selector emits at most its first **qualifying** event per symbol/session. An earlier raw fresh-low anchor that never qualifies a selector does not consume that selector; a fire in one selector does not consume another selector; once a selector fires, a second event for that same selector/session is refused. Different selectors may reference the same candidate anchor; dependence is handled at the date/episode level rather than pretending those rows are independent trades.
...
The price-reference entry bar must itself have positive volume. Outcome/MFE/MAE path bars are price-evidence bars and therefore require positive volume; otherwise that horizon is censored rather than treating a zero-volume aggregate as executable path evidence.
...
- candidate-anchor confirmation delay cost `(entry_open - candidate_low)/ATR20`;
- episode-low confirmation delay cost `(entry_open - minimum_low_from_candidate_through_confirmation)/ATR20` for confirmed families;
...
Two co-equal LOD diagnostics are separate from local-return outcomes; neither is the singular primary anchor. **Candidate-anchor survival** requires no subsequent eligible **positive-volume** RTH low below the original candidate low through close. For confirmed families, **episode-low survival** freezes the minimum low observed on positive-volume bars from candidate through confirmation and asks whether that low survives from confirmation through close. A profitable local bounce may fail either LOD label, and a final LOD may still offer poor entry quality. These labels are never collapsed into one success boolean.

## 10. Deterministic matched controls

The primary comparison is not an unconditional hit rate. Build a separate BASE-candidate control census using the **first lawful fresh-low anchor per symbol/session/30-minute bin**. This bin-local census is independent of per-selector first-qualifying-event identity. The census is a matching substrate, not a sixth selector or trial cell, and its membership uses candidate-time facts only. For each selected event, build its control pool from this census **without using outcomes or later selector labels**.

A control must have the same ticker, the same early/late retrospective partition, the same 30-minute decision-time bin, the same displacement bucket `[0.50,0.75), [0.75,1.00), [1.00,1.50), [1.50,+inf)`, the same sign of QQQ open-to-decision return, and the same confirmation-delay bar count used by the selected event. The selected date itself is excluded. A control is **not** excluded because it later satisfies RECLAIM, EXHAUSTION or CONTINUATION; future family membership is forbidden as a matching variable.

For a confirmed selector with delay `d` bars, the matched base control is shifted by the same `d` completed confirmation-delay bars **plus the same one full five-minute processing-latency bar** before taking its price-reference entry. Immediate selectors also receive the one-bar processing latency. This prevents a selected family from being compared with an artificially earlier baseline entry.
...
V4 registered selector grid, unchanged from v1/v2/v3: BASE_FRESH_LOW, EXHAUSTION_FORMING, RECLAIM_ONLY, EXHAUSTION_RECLAIM, CONTINUATION_RISK × 30m/60m/120m/close × 10/25/50 bp = **60 cells**.
...
Implementation tests must prove that there is no global first-event-per-symbol/day gate, that an unqualified early anchor cannot consume a later qualifying event for that selector, that one selector firing cannot consume another selector, that the control census remains bin-local and independent of selector identity, that the decision-time bar open itself is never used as the entry reference, that selected and control entries both occur exactly one full five-minute processing bar after their respective decision/confirmation clocks, and that changing any future bar after candidate decision cannot alter candidate identity or forming features; changing bars after confirmation cannot move the confirmation timestamp earlier; changing a candidate's **future family label** cannot change whether it appears in the control census; a future duplicate/corrupt row cannot erase an earlier lawful candidate; and a missing/corrupt outcome bar censors only that outcome rather than deleting the fire.
...
Before any empirical run: commit this v4 preregistration and `tti_r1b/config_v4.json`; record exact hashes; reconcile R1-A/shared TrialLedger state; then append the complete 60-cell v4 budget through the canonical TrialLedger writer. V1–v3 remain immutable DO_NOT_RUN audit artifacts.
