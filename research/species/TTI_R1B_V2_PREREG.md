# TTI R1-B v2 preregistration — exhaustion/reclaim versus continuation

**Status: v2 candidate preregistration; NOT REGISTERED; NOT RUN.**

V2 supersedes the **unregistered/unrun** v1 freeze at `0c54e10e20c43827802d3d829505f74f57f8eb5f` after pre-outcome adversarial review. `research/species/tti_r1b/V1_ADVERSARIAL_REVIEW.md` records the three corrections. No price threshold, selector, horizon or cost cell changed; v2 fixes control/candidate identity and diagnostic bookkeeping before any trial registration.
...
Two LOD diagnostics are separate from local-return outcomes. **Candidate-anchor survival** requires no subsequent eligible RTH low below the original candidate low through close. For confirmed families, **episode-low survival** freezes the minimum low observed from candidate through confirmation and asks whether that low survives from confirmation through close. A profitable local bounce may fail either LOD label, and a final LOD may still offer poor entry quality. These labels are never collapsed into one success boolean.
...
V2 registered selector grid, unchanged from v1: BASE_FRESH_LOW, EXHAUSTION_FORMING, RECLAIM_ONLY, EXHAUSTION_RECLAIM, CONTINUATION_RISK × 30m/60m/120m/close × 10/25/50 bp = **60 cells**.
...
Before any empirical run: commit this v2 preregistration and `tti_r1b/config_v2.json`; record exact hashes; reconcile R1-A/shared TrialLedger state; then append the complete 60-cell v2 budget through the canonical TrialLedger writer. V1 remains an immutable DO_NOT_RUN audit artifact.
