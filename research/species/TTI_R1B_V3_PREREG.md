# TTI R1-B v3 preregistration — exhaustion/reclaim versus continuation

**Status: v3 candidate preregistration; NOT REGISTERED; NOT RUN.**

V3 supersedes the **unregistered/unrun** v2 freeze at `1348a238c8fbcb79b617478311f2ce0e1e37d4c0` after a second pre-outcome adversarial review found a zero-processing-latency entry reference. `research/species/tti_r1b/V2_ADVERSARIAL_REVIEW.md` records the correction. V1 and v2 are immutable DO_NOT_RUN audit artifacts. No market-state threshold, selector, horizon or cost cell changed.

V2 had already superseded the unregistered v1 freeze after correcting future-label control conditioning, selector identity and low-anchor bookkeeping. Those corrections remain in v3 unchanged.
...
Each selector emits at most its first **qualifying** event per symbol/session. An earlier raw fresh-low anchor that never qualifies a selector does not consume that selector; once a selector fires, a second same-selector event that session is refused. Different selectors may reference the same candidate anchor; dependence is handled at the date/episode level rather than pretending those rows are independent trades.
...
V3 registered selector grid, unchanged from v1/v2: BASE_FRESH_LOW, EXHAUSTION_FORMING, RECLAIM_ONLY, EXHAUSTION_RECLAIM, CONTINUATION_RISK × 30m/60m/120m/close × 10/25/50 bp = **60 cells**.
...
Before any empirical run: commit this v3 preregistration and `tti_r1b/config_v3.json`; record exact hashes; reconcile R1-A/shared TrialLedger state; then append the complete 60-cell v3 budget through the canonical TrialLedger writer. V1 and v2 remain immutable DO_NOT_RUN audit artifacts.
