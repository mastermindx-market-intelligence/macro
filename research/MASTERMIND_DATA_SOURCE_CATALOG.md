...
Datasets whose code is complete, whose docstrings read like shipped capability, and whose store had zero
rows at the original census. **A catalog that listed these alongside live stores would be exactly the lie
this project exists to make impossible.** All original existence checks ran 2026-08-12 against the DATA
checkout. **Update 2026-09-12:** the CN TuShare row is no longer zero-row: a private self-hosted canary
store exists and exact-head run `33043190487` completed. Its full historical range campaign is still
incomplete, so this update does not reclassify it as a production-complete corpus.

| Dataset | Declared by | Store status `[V]` | What breaks if you assume it exists |
|---|---|---|---|
| **CN TuShare full-A spine** — `daily`, `daily_basic`, `stk_limit`, `suspend_d`, `stock_st` + `reference/{security_master,identity_aliases,instrument_classification}` | `collectors/china_tushare_spine.py` (~3,600 lines), contract `research/CN_TUSHARE_FULL_A_SPINE_CONTRACT_2026-08-08.md` | **2026-08-12 census:** declared root absent in the DATA checkout. **2026-09-12 update:** private self-hosted store is materialized; exact-head run `33043190487` completed with 6 requests, zero failures, `stage=complete`; full range campaign / sanitized completeness manifest not yet complete. | The `DNR:KILL-CN-ADJUSTED-TAPE-LEGAL-LIMIT` reopen path routes through this store. The TECHNICAL readiness gate is separately promoted on live-canary + unchanged #5523 adversarial range evidence, but that authorizes collection rather than certifying a complete corpus. TuShare licensing/compliance is `CHAIRMAN_VERIFIED_PRIVATE / SATISFIED` and is never a runtime gate (`DEC:CNLI-TUSHARE-COMPLIANCE-IS-CHAIRMAN-VERIFIED-PRIVATE`). |
