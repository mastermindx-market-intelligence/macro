...
**Verified at:** macro `origin/main` (this branch's base). All `file:line` anchors in §2.1–§2.4 and §3 were re-verified fresh at write time with `wc -l` / `grep -n`. §2.5 anchors are coordinates on the unmerged sibling branch `claude/mo-a-a1-a-f02-1` and are explicitly NOT verified against this branch's base (see §2.5 and the correction note below).
