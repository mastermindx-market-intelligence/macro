...
- `wmt_incomplete_recipe.json` records only the supported discovery anchors `WMT`, 720 minutes,
  named `extended`, and `extended_hours_enabled=true`; every absent identity, exact source reference,
  session grammar, adjustment, observed-indicator and export field is enumerated in `missing_fields`.
- `silver_incomplete_recipe.json` records only the 480-minute discovery anchor. The word “silver”
  remains descriptive in the recipe ID and evidence record; `instrument.display_symbol` is null.
  Product class, exact symbol, vendor/feed, contract or roll, session, extended-hours state, chart
  type, adjustments, source reference, indicator provenance and export remain explicitly missing.
