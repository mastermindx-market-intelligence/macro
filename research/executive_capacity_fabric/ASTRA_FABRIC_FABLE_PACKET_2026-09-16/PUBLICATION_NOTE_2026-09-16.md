...
## Stale-truth corrections at publication (2026-09-16 ~08:25Z)
...
| Packet R684 "Verified state": #684 at `9d47bbec…` | #684 head is now `bede4097…` (above). The R684 review packet's scope and pinned head must be re-verified against the current PR before any placement; the packet itself says so ("Revalidate only changed state and reviewer absence before admission"). | root reply 4 |
