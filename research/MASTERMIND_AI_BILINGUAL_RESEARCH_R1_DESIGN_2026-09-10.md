...
These are the frozen boundaries of the lexical R1 wave. The approved R1B successor may modify the existing corpus owner and minimally register its newly introduced suites plus an ownership regression under existing pre-merge `gate: code` owners, but only under the explicit authority and fresh collision receipt in the R1B design. No other R1 boundary is lifted.
