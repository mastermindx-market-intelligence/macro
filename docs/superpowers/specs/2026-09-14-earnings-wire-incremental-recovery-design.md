...
The 10/10 state is a bounded hourly refresh whose work scales with the current public estate plus genuinely new/corrected forward events, not the complete transcript archive. Completion requires live production to advance beyond 2026-07-29, not merely green tests or a merged PR.
...
The redacted catalog advances from `earnings.public_wire_routes/v1` to `earnings.public_wire_routes/v2` by adding `forward_selection_floor_date`.
...
The catalog's existing `source_generation_id` and `source_manifest_sha256` continue to identify the last accepted story generation. They are sufficient to retrieve and verify the prior immutable manifest. The new date is selection policy metadata, not event truth.
...
8. Fail closed if any newly added key lacks a valid date; otherwise historical additions before the floor are deliberately skipped.
9. Hydrate, receipt-check, contract-validate, and apply the existing evidence admission policy only to the selected keys.
10. Build the complete public manifest from selected articles, preserving admitted survivors and adding newly eligible calls.
...
Current marker/immutable mismatch, prior-receipt mismatch, missing transcript dates, unsafe future dates, selected-count overflow, packet receipt mismatch, or contract failure aborts before publication. A source failure may retain a verified existing publication for at most 48 hours. When that fallback is unavailable, the raised message includes both the causal fresh-source error and the stale-fallback error; the cause is never masked again.
...
The builder loader accepts v1 for migration and v2 thereafter; the writer emits v2. The ticker-dossier consumer accepts both v1 and v2 so the schema rollout cannot silently demote exact record links to the generic archive during deployment or rollback. Unknown schemas remain rejected.

## Acceptance and production proof

Tests must prove selection excludes pre-floor backfill, includes same-day and post-floor additions, includes all corrections, rebuilds admitted routes, removes corrected-held routes, enforces bounds, migrates v1 state, preserves the floor, and retains the causal error.
