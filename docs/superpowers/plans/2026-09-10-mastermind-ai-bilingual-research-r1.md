...
> **Successor authority — R1B (2026-09-14):** The path restrictions and protected-boundary checks below govern the original lexical R1 task. The separately approved source-bound-evidence successor in `research/MASTERMIND_AI_SOURCE_BOUND_EVIDENCE_R1B_DESIGN_2026-09-13.md` supersedes them only for its explicit `engine/research_vault/corpus.py` owner extension and the minimal pre-merge `gate: code` ownership registration for the two newly introduced product suites and their ownership regression. That exception does not reopen any other R1 non-goal or authorize another job, workflow, dependency, retrieval plane, quota plane, or product authority.
