...
## R14 — preserve the News gate without unrelated template fanout

Chairman continuation/review waiver #7953/comment5846527711 remains effective. Protected Skillpack Mastermind@4c6b206d3fb7fbc6d077faf61ae361bedf259925, compatible1.0.1/bootstrap1; required procedure bodies unchanged. Same #7982 branch; no Codex or Executive.

CI36273467401/job108501540847 failed test_ci_pack::test_exclusive_curation_narrows_ordinary_code_prs: templates/index.html selected135 jobs against134. Exact tested merge0cad72c0e4aaa1c044f0350209608aa022e2512c, baseaca25ac324099b1b06fa4cd643e28582aac936dc. Its exact-base replay exhausted900 seconds; no inherited-red waiver is inferred.

Root cause proven with the actual scope engine: adding our real ticker-builder tests to earnings-release-identity introduces templates/** through lib/pages and lib/seo; the pre-News job did not select templates/index.html. Thirty first-party old-job source files were authenticated against the tested Git tree before the comparison.

Final repair keeps BOTH guidance suites in the SAME source-gated earnings-release-identity job, with identical steps, dependencies, runner, gate:code and weight. Its now-explicit exclusive scope retains all30 incumbent inferred ownership paths plus the new suites'56 named dependencies and concrete ticker templates:72 unique paths. The actual selector retains every prior/named subject and no longer selects this job for the unrelated landing page. No job, runner, scheduler, scope-engine implementation or CI ceiling changes. No test skipped or product code changed.

The initially considered move into conviction-profile was rejected BEFORE source publication: that job deliberately omits requests to preserve its valuation producer's collector-free environment. The final repair leaves that job byte-for-byte unchanged relative to the incoming candidate (including the earlier R8 formatter path). An earlier local text-edit placement error was also discarded. Neither draft is shipped or credited as proof.
