...
OUT=ROOT/'mockups/evidence/risk-radar-integration-20260919'
...
class Driver:
 ...
 def capture(self,*,url,cell,timeout_s):
  ...
  try:
   ...
   page.evaluate("""() => {const d=document.getElementById('dlg-risk'); d.classList.add('open','mx5-dlg-in'); document.body.style.overflow='hidden';} """)
...
try:
 result=cap.main(['--registry',str(ROOT/'data/product_experience/page_registry.json'),'--base-url',url,'--routes','macro.html','--output-dir',str(OUT),'--manifest',str(OUT/'manifest.json'),'--smells',str(OUT/'smells.json'),'--emit-md',str(OUT/'REPORT.md'),'--settle-ms','300','--delay-ms','0','--timeout-s','40','--force-state','radar_open:.open','--force-state','radar_focus:focus(#dlg-risk .gde-disc > summary)'],driver_factory=Driver)
