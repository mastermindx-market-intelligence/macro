...
PR #7048 round-2 heal. Crops of `scripts.build_vector._hub_html` after
the origin/main merge and the helper-glance repair: Explore band, main's
later chip face, H1 hover scale, snapshot clock-wrap, and the plain
what-changed receipt. These crops show that presentation on the real
theme/lang mechanism.

## Cells

| file | theme | lang | viewport | PNG (clip) | toggle |
|---|---|---|---|---|---|
| `start-tier1-dark-en-1440.png` | dark | en | 1440×900 | 1326×544 | data-theme=dark data-lang=en |
| `start-tier1-dark-en-390.png` | dark | en | 390×844 | 386×643 | data-theme=dark data-lang=en |
| `start-tier1-dark-zh-1440.png` | dark | zh | 1440×900 | 1326×555 | data-theme=dark data-lang=zh |
| `start-tier1-dark-zh-390.png` | dark | zh | 390×844 | 386×653 | data-theme=dark data-lang=zh |
| `start-tier1-light-en-1440.png` | light | en | 1440×900 | 1326×545 | data-theme=light data-lang=en |
| `start-tier1-light-en-390.png` | light | en | 390×844 | 386×643 | data-theme=light data-lang=en |
| `start-tier1-light-zh-1440.png` | light | zh | 1440×900 | 1326×555 | data-theme=light data-lang=zh |
| `start-tier1-light-zh-390.png` | light | zh | 390×844 | 386×653 | data-theme=light data-lang=zh |
...
- **(a) Explore chip row** — Bitcoin Vector pills
  ("Risk on" / "Momentum negative" · 「风险偏好」/「动量偏弱」) and
  Bonds ("Bond health" / 「债券健康」). No health bar. Bonds icon is
  the de-emoji "≋", not a temple glyph. Band heading is Explore / 探索.
- **(b) Hero snapshot clock** — JS finds `data-asof` and stamps
  `is-live`, revealing the baked civil date
  ("Latest market snapshot · Sep 18, 2026" /
  「最新市场快照 · 2026年9月18日」). No viewer-clock tick. The
  `no-clock` failsafe keeps the same static snapshot word when the
  live stamp never lands.
...
Product CSS at `max-width:560px` hides `.nav.vc .chips` and compact-
collapses the vector cards. Capture injects a documented override so
the chip strings can be measured at 390 rather than vanishing.

Visual on the Bitcoin Vector `.chips` after the override
(`flex-wrap: wrap`): both languages fit on one row at 390. Wrap is
armed and would catch a future longer string; it does not fire on
these pills.
...
Capture script (not committed): `.capture_start_tier1.py` in the lane
worktree. Clock class observed on every cell: `hub-clock-wrap is-live`.
...
- At 390, a capture-only stylesheet un-hides `.nav.vc .chips` and
  restores column card geometry so wrap/clock can be seen. Live product
  still compact-hides those at `max-width:560px`. The capture also sets
  `#hub-views` `data-view=vc` so Explore is the mobile pane (default is
  Markets).
...
- Theme CSS is the repo file (`templates/theme.css` + self-hosted
  `templates/fonts/Inter-*.woff2`), served next to the rendered HTML.
  Inter loads; no Google Fonts.
