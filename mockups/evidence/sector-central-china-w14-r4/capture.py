#!/usr/bin/env python3
"""S1-rig evidence matrix for sector_central_china W14 r4 (heal recapture).
...
reduced-motion, settle-to-opacity-1, computed-style text. Refuses a dirty
tracked tree and stamps `git rev-parse HEAD` at capture time so a working-tree
edit cannot wear a prior committed sha.

Recapture (porcelain-empty tracked tree required):

    python3 mockups/evidence/sector-central-china-w14-r4/capture.py
"""
...
def _prep(page, spec: dict) -> None:
    ...
    _set_view(page, view)
    kind = spec["kind"]
    if kind in ("table8", "sigma", "p0", "explore", "silinks"):
        ...
        page.evaluate(
            """(zh) => {
              ...
              if (el && !el.textContent.trim()) {
                el.textContent = zh ? '截至 2026-09-11' : 'as of 2026-09-11';
              }
            }""",
            spec.get("locale") == "zh",
        )
    if kind == "silinks":
        _set_view(page, "explore")
        page.wait_for_selector(".si-links", timeout=8_000)
    if kind == "empty":
...
def _settle(page, sel: str | None) -> None:
    target = sel or "body"
    page.wait_for_function(
        """(sel) => {
          const el = document.querySelector(sel);
...
def _cell_specs() -> list[dict]:
    ...
    for theme in ("dark", "light"):
        ...
        for locale in ("en", "zh"):
            ...
            crop(
                f"confluence-{theme}-{locale}-1440",
                "confluence",
...
def _write_readme(head: str, shots: list[dict], hscroll: list[dict], generated: str) -> None:
    ...
    families = [
        ...
        ("(i) .si-links band (LENS ? + Reversal Sleeve)", "silinks"),
    ]
    deg = [
    ...
    lines += [
        ...
        "",
        "r3 `error-light-en-1440` (`7431fc8347511fe5.png`) was the identical "
        "content-addressed file as r2's `mxerror-light-en-1440`. This round recaptured "
        "error in both locales; the light-EN abort path is still `7431fc8347511fe5.png` "
        "(deterministic rig, same bytes). ZH error is a new file.",
        "",
...
def main() -> int:
    ...
    for banned in (
        ...
        if banned in html:
            raise SystemExit(f"fixture still carries banned {banned!r} — refusing capture")

    staging = Path(tempfile.mkdtemp(prefix="scchina-w14r4-"))
    try:
        ...
        with sync_playwright() as pw:
            ...
            for spec in specs:
                ...
                page.add_init_script(
                    ...
                    f"  localStorage.setItem('cnfw-btbl-mode', {json.dumps('sigma' if spec['kind']=='sigma' else 'rel')});\n"
                    "  localStorage.setItem('cnfw-btbl-sort-v2', JSON.stringify({col:'20d',dir:-1}));\n"
                    "} catch (e) {}\n"
                )
                ...
                if spec["kind"] == "sigma":
                    ...
                    tabs = page.evaluate(
                        "() => (document.getElementById('btbl-mode')?.innerText || '')"
                    )
                    if spec["locale"] == "zh" and "反常程度" not in tabs:
                        raise SystemExit(f"sigma ZH missing 反常程度: {tabs!r}")
                    if spec["locale"] == "en" and "How unusual" not in tabs:
                        raise SystemExit(f"sigma EN missing How unusual: {tabs!r}")
                if spec["kind"] == "table8":
                    more = page.evaluate(
                    ...
                    heads = page.evaluate(
                        "() => [...document.querySelectorAll('#btable thead th')].map(t => t.textContent).join(' ')"
                    )
                    if spec["locale"] == "zh" and ("1日" not in heads or "月至今" not in heads):
                        raise SystemExit(f"table8 ZH missing 1日/月至今: {heads!r}")
                if spec["kind"] == "silinks":
                    text = page.evaluate(
                        "() => (document.querySelector('.si-links')?.innerText || '')"
                    )
                    if spec["locale"] == "zh" and "反转组合" not in text:
                        raise SystemExit(f"silinks ZH missing 反转组合: {text[:200]!r}")
                    if spec["locale"] == "en" and "Reversal Sleeve" not in text:
                        raise SystemExit(f"silinks EN missing Reversal Sleeve: {text[:200]!r}")
                if spec["kind"] == "footer":
    ...
    manifest = {
        ...
        "tool": "mockups/evidence/sector-central-china-w14-r4/capture.py",
        ...
        "honesty": {
            ...
            "gaps": (
                "P0 crop clones the LIVE rel-mode tab and the 5-day-rotation .sub into "
                "#p0-csi-frame so both CSI-300 strings share one frame (they do not sit "
                "together at rest) — token-truth only, never hierarchy/layout. "
                "Confluence as-of text is seeded when the lazy board is not mounted "
                "(locale-true: 'as of' / '截至'). "
                "r3 error-light-en-1440 reused r2's content-addressed PNG "
                "7431fc8347511fe5; this round recaptured error in EN and ZH — light-EN "
                "is still 7431fc8347511fe5 (deterministic abort path), ZH is new. "
                "si-links is recaptured this round (light-ZH and 390 included). "
                "LIGHT DESKTOP composition is in the rest-light-*-1440 cells."
            ),
        },
        "axes": {
            "access": ["anonymous"],
    ...
    (OUT / "EVIDENCE.yml").write_text(
        ...
        "manifest: mockups/evidence/sector-central-china-w14-r4/manifest.json\n",
        encoding="utf-8",
    )
