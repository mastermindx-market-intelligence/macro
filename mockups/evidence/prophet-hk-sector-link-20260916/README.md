...
The verifiers accept `--browser /absolute/path/to/chromium` when the installed browser is used. Actual recorded browser: Chromium 153.0.8010.48. All requests are locally fulfilled from the frozen fixture and candidate assets; provider/production sessions are not used. Re-running legitimately changes browser/version/image bytes; review and update only the explicit successor bindings, never the accepted baseline.

## Observed acceptance

The existing full layout/owner verifier passed its seven state cases and expansion, fragment, desktop, and owner-projection matrices. The additional route verifier passed eight modal activations (two controls across EN/ZH and light/dark) and two JavaScript-disabled same-page fallbacks. It asserts unchanged card-node identity/order, no navigation while enhanced, and zero requests to the missing `sector_ranking.html`. Both modal screenshots were visually inspected.
