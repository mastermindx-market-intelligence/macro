# Fable Mode — the engineering doctrine (worker standard)

**What this file is.** The complete Revision 2 working doctrine for hands-on engineering and research work — opening a problem (§2), investigating (§3), designing (§4), executing and delegating (§5), stalling (§6), reporting and landing (§7), the pre-send gate (§8), the failure catalog (§9), antipattern signatures (§10). Section numbers are stable and cited from outside this skill (for example `research/MASTERMIND_AGENT_OS_ARCHITECTURE.md` cites §2.2); never renumber them.

**Who reads it.** Every model doing bounded work of the build, debug, analysis, or review class — a worker, a lane, or a seat executing under the continuation law — reads this in full at task start. Extract, draft, and census lanes do not: they get the return packet and the turn-ending clause pasted into their commission (`harness-adapters.md` section 7), because ~13k tokens of resident doctrine is the wrong price for a format sweep. A seat that only commissions reads `../SKILL.md` §2 (the one-page summary) and comes here when it writes a `NOT DONE UNLESS` clause, judges a return against one, or picks up a tool itself.

**Harness caveat.** Two rules below name a stash as a checkpoint (§5.4, §7.1). In this fleet the stash stack is shared across every worktree and session, so a bare `git stash` / `git stash pop` is forbidden: use a WIP commit, or `git stash push -u -m <unique-tag>` restored by SHA with `git stash apply` — never `pop`. Where any command here conflicts with the repository's `CLAUDE.md` / `AGENTS.md`, those win.

**Relationship to the core.** `../SKILL.md` Revision 3 owns the seat's doctrine (orchestration, long horizon, adjudication, the extended gate) and refers to the rules here by § number. The six commitments in §1 below are the same six the core restates; the core adds four seat commitments that this file does not repeat.
...
Trigger: an edit will touch more than ~3 files, or any scripted transformation (repo-wide rename, codemod, regex replace, formatter run). Before the first edit, create a one-command revert point — a WIP commit, or a tagged `git stash push` restored by SHA (never a bare stash on a shared stack) — and write down the exact undo command. Mid-transformation discoveries ("the regex also matched the vendored dir") then cost one revert instead of a hand-unpicking session that introduces its own errors. No checkpoint, no bulk edit.
...
*Revision 3 (2026-09-24).* This text is Revision 2 moved verbatim into `references/engineering.md` so the skill's core (`../SKILL.md`) can carry the seat doctrine — orchestration, long horizon, adjudication — without pushing the worker standard out of a reader's context budget. No rule was renumbered. Two phrases were amended for the shared-stash law (§5.4's "named stash" and §7.1's "`git stash`" now say a WIP commit or a tagged stash push, never a bare stash); everything else is verbatim, and the header above and this note are the only additions. Two Revision 2 rules are restated with more force in the core because seats break them at program scale: §5.8 (the cold-stranger test, now the commission packet in `packets.md`) and §5.10–§5.11 (open the artifact; pending work has no result — now O.10 and the ladder in `long-horizon.md`).
