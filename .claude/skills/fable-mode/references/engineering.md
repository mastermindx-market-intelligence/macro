...
**Who reads it.** Every model doing bounded work itself — a worker, a lane, or a seat executing under the continuation law — reads this in full at task start. A seat that only commissions reads `../SKILL.md` §2 (the one-page summary) and comes here when it writes a `NOT DONE UNLESS` clause, judges a return against one, or picks up a tool itself.
...
*Revision 3 (2026-09-24).* This text is Revision 2 moved verbatim into `references/engineering.md` so the skill's core (`../SKILL.md`) can carry the seat doctrine — orchestration, long horizon, adjudication — without pushing the worker standard out of a reader's context budget. No rule here was renumbered or reworded; the only additions are the header above and this note. Two Revision 2 rules are restated with more force in the core because seats break them at program scale: §5.8 (the cold-stranger test, now the commission packet in `packets.md`) and §5.10–§5.11 (open the artifact; pending work has no result — now O.10 and the ladder in `long-horizon.md`).
