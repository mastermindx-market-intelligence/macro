...
**Classify before acting** (S.2): RUNNING (alive, within budget) → nothing. DELIVERED → judge. SILENT (alive, past budget, no artifact) → check process state before diagnosing: a lane under a long mandated command shows exactly the signature of a stall. DEAD (process gone, no artifact) → read its last output; relaunch only with a changed input or a named reason the outcome will differ (§6.1). THRASHING (alive, hundreds of identical actions, record untouched across two cycles) → report it with evidence and stop it through the sanctioned path; never kill by pattern.
...
**Notifications and nudges.** A "completed" notification is at most DELIVERED and often less: read the final message. If it is a status note, send ONE nudge demanding the packet — and plan for the nudge failing. Do not spawn a replacement for the same question; you pay full price again for the same risk. Instead, answer the gating sub-questions yourself with a few targeted probes in parallel with the worker.
