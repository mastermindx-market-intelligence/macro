...
# News controls: real source-owned locale event, capped counts and filter semantics.
def test_news_controls_are_named_native_pressed_buttons():
    html = _render_full()
    assert 'id="nxSeg" role="group"' in html
    assert 'data-label-en="Filter news by topic"' in html
    buttons = re.findall(r'<button[^>]*data-lane="([^"]+)"[^>]*>', html)
    assert len(buttons) == 5
    for tag in re.findall(r'<button[^>]*data-lane="[^"]+"[^>]*>', html):
        assert 'type="button"' in tag and 'aria-pressed=' in tag
    assert 'role="status"' in re.search(r'<span[^>]*id="nxCount"[^>]*>', html).group(0)


def test_news_search_indexes_the_displayed_translation_and_escapes_it():
    vm = _full_vm()
...
def test_news_dynamic_locale_uses_the_existing_document_event():
    ...
    assert 'data-label-zh="搜索新闻"' in src


def test_news_empty_search_has_native_recovery_and_touch_controls():
    html = _render_full()
    assert 'id="nxReset"' in html
    assert 'Clear filters' in html and '清除筛选' in html
    assert 'min-height:40px' in html
    assert '.seg-btn:focus-visible' in html


def test_news_count_and_locale_follow_actual_visible_results():
    import json
    import shutil
    import subprocess
...
class E {
 constructor(attrs={}) {this.attrs=attrs;this.handlers={};this.classes=new Set();this.style={};this.textContent='';this.value='';}
...
const seg=new E({'data-label-en':'Filter news by topic','data-label-zh':'按主题筛选新闻'});
seg.querySelectorAll=()=>buttons;seg.querySelector=()=>buttons[0];
const search=new E({'data-ph-en':'Search headlines, tickers…','data-ph-zh':'搜索标题、代码…','data-label-en':'Search news','data-label-zh':'搜索新闻'});
const count=new E(),empty=new E(),more=new E(),reset=new E();
...
const ids={nxFeed:feed,nxSeg:seg,nxSearch:search,nxCount:count,nxEmpty:empty,nxMore:more,nxReset:reset};
document.getElementById=id=>ids[id]||null;document.querySelectorAll=q=>q.startsWith('.rel-time')?[stamp]:[];
global.window=new E();
"""
    checks = r"""
assert.equal(stories.filter(s=>!s.classes.has('hide')).length,24);
assert.equal(count.textContent,'24 of 30 headlines');
assert.equal(buttons[0].getAttribute('aria-pressed'),'true');
more.emit('click');assert.equal(count.textContent,'30 headlines');
buttons[3].click();assert.equal(count.textContent,'5 headlines');
...
search.value='missing';search.emit('input');assert.equal(count.textContent,'0 headlines');
...
assert.equal(seg.getAttribute('aria-label'),'按主题筛选新闻');assert.equal(count.textContent,'0 条');
assert.ok(stamp.textContent.includes('小时前'));assert.equal(search.value,'missing');
reset.emit('click');assert.equal(search.value,'');assert.equal(buttons[0].getAttribute('aria-pressed'),'true');
assert.equal(document.activeElement,search);assert.equal(count.textContent,'显示 24 / 30 条');
search.value='芯片';search.emit('input');assert.equal(count.textContent,'30 条');
root.setAttribute('data-lang','en');document.emit('langchange');assert.equal(count.textContent,'30 headlines');
...
def test_news_guest_state_does_not_invent_an_expired_session():
    html = _render_full()
...
def test_news_gated_panels_honor_hidden_state_in_flex_and_grid():
    html = _render_full()
    assert '#nxIntel [hidden]' in html and '#nxWire [hidden]' in html
    assert '#nxIntel[hidden]' in html and '#nxWire[hidden]' in html
    assert '#nxIntel[data-state="quiet"] .nxi-health{display:none}' in html
    assert 'display:none!important' in html


def test_news_desk_intro_is_short_and_does_not_claim_live_before_loading():
    html = _render_full()
    assert 'Live story graph' not in html
    assert 'Story updates' in html
    assert 'Developing stories with their sources and market context.' in html
