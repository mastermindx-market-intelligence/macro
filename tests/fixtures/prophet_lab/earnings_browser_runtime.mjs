...
if(scenario==='anonymous'){session=null;await open();assert.equal(requests.length,0);noEvidence();assert.match(controls.status.textContent,/Sign in/);controls.signin.emit('click');assert.equal(opened,1);}
