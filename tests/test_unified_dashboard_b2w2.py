...
Pins the binding contract per DEC-SPINE-SCALE-BINDINGS (META-CEO A, 2026-09-20):
  ...
  · CN row (data-market="cn", data-blocked-feed="cn_market_state") binds to
    ...
    by build_china to data/cn_market_state/latest.json. Same vm shape.
...
def _intl_ms_view(intl_root: Path, market_key: str) -> dict | None:
    """Mirror scripts/build_site.py:_intl_ms_view — load the persisted snapshot
    + the parquet log into the macro vm entry shape the template reads."""
