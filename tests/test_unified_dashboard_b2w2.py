...
Component assertions are scoped to the spine slice: HK/CN rows carry
data-state="real" + marker geometry + caveat present in the disclosure;
bonds/commodities rows stay designed-null with renamed-or-kept slugs.
The committed macro route intentionally keeps this candidate off-route; a
separate built-page assertion below pins that composition boundary.
"""
...
# --------------------------------------------------------------------------- #
# Built-page composition proof — the B2W2 candidate remains fully tested above
# as an isolated component, but the primary US macro route intentionally does
# not mount it while the compact regime-radar remains canonical.
# --------------------------------------------------------------------------- #
...
def test_committed_macro_html_keeps_b2w2_candidate_off_primary_route():
    """The served macro page must not leak the retained spine candidate."""
