...
def test_frozen_cn_hk_ca_ledgers_replay_without_mutation(tmp_path) -> None:
    from engine.risk_radar_intl_evidence import derive_evidence

    # These exact expected counts belong to the reviewed P2 input vintage, not
    # whichever nightly ledger happens to be in an integration checkout.
    fixture_path = Path(__file__).parent / "fixtures" / "risk_radar_intl_p2_20260924.json"
    fixture = json.loads(fixture_path.read_text())
    assert fixture["source_head"] == "5d6ae511c7b82b05e175ba7ea36e2fc5003b6502"
    repo_root = tmp_path
    for record in fixture["ledgers"].values():
        raw = record["text"].encode()
        assert hashlib.sha256(raw).hexdigest() == record["sha256"]
        target = repo_root / record["source_path"]
...
def test_parent_single_decline_density_counterexample_stays_refused() -> None:
    """Enroll PR7622's independent pre-P2 RED case in the owning CI suite."""
    from research.grey_deer.probe_china_alert_independence import examine

    result = examine()
    sample = result['synthetic']
    assert sample['single_continuous_decline'] is True
    assert sample['daily_alert_calls'] == 40
    assert sample['maximum_disjoint_21_session_alert_windows'] == 2
    assert sample['one_alert']['can_force'] is False
    assert sample['dense_daily_alerts']['can_force'] is False
    assert sample['dense_daily_alerts']['n_loud_episodes'] == 1
    assert sample['dense_daily_alerts']['n_independent_episodes'] == 5
