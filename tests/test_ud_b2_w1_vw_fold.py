...
def _slice_vol_weather_strip(isle_html: str) -> str:
    ...
    """
    attr_pos = isle_html.find("data-sx-vw-strip")
    if attr_pos < 0:
        return ""
    # Walk backwards to the strip's opening <div
    div_start = isle_html.rfind("<div", 0, attr_pos)
    if div_start < 0:
        return ""
    ...
    while i < n and isle_html[i] != ">":
        i += 1
    i += 1  # step past the `>`
    while i < n:
        if isle_html.startswith("<div", i) and (
            isle_html.startswith("<div ", i) or isle_html.startswith("<div>", i)
        ):
            depth += 1
            i += 5
        elif isle_html.startswith("</div>", i):
            depth -= 1
            i += 6
            if depth == 0:
                return isle_html[div_start:i]
        else:
            i += 1
...
class TestUDB2W1VwFold:
    ...
    def test_plain_word_names_inside_isle(self, macro_html):
        ...
        glance = _slice_vol_weather_strip(isle)
    ...
    def test_ok_chip_with_extreme_band_shows_storm_tier(self, vol_weather, macro_html):
        ...
        for k in extreme_keys:
            row_pat = re.compile(
                ...
                rf'data-sx-vw-chip="{re.escape(k)}"[^>]*>(?P<body>.*?)</div>\s*</div>\s*<div\s+class="sx-vw-tier">'
                rf'(?P<tier>.*?)</div>',
                re.DOTALL,
            )
            m = row_pat.search(isle)
            assert m, (
                f"Extreme-band row for chip={k!r} not found in isle"
            )
