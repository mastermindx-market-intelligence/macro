...
# Flagship R0: offline prompt/fixture contracts, not model-quality proof.
...
def test_financial_scope_preserves_unrelated_roleplay_boundary():
    scope = gw._BRAIN_SYSTEM_PROMPT.split("SCOPE — THIS PRODUCT ONLY:", 1)[1]
    scope = scope.split("PROPRIETARY — NEVER REVEAL OR DISCUSS:", 1)[0]
    assert "unrelated coding, creative-writing, role-play" in scope
    assert "long text unrelated to this product regardless of framing" in scope

def test_financial_scope_preserves_brief_refusal_budget():
    assert "Refusals are ONE sentence — never spend tokens on them" in gw._BRAIN_SYSTEM_PROMPT
