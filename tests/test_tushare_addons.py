...
def test_gold_premium_rebuilds_and_reaudits_when_rebase_changes_material_inputs():
    ...
    assert "GOLD_CANONICAL_OUTPUT_PATHS=(" in text
    assert "gold_restore_origin_main_outputs()" in text
    ...
    rebuild_guard = "if ! python -m scripts.build_commodities; then"
    normalize = "python -m scripts.inject_data_base"
    audit = "python -m scripts.audit_china_gold_premium --strict-render"
    audit_failed = 'if [ "$gold_post_rebase_audit_rc" -ne 0 ]; then'
    restore = "gold_restore_origin_main_outputs"
    ...
    assert post_rebase.index(changed) < post_rebase.index(rebuild_guard)
    assert post_rebase.index(rebuild_guard) < post_rebase.index(normalize)
    assert post_rebase.index(normalize) < post_rebase.index(audit)
    assert post_rebase.index(audit) < post_rebase.index(audit_failed)
    assert post_rebase.count(restore) >= 2
    assert post_rebase.index(audit_failed) < post_rebase.rindex(restore)
    assert post_rebase.rindex(restore) < post_rebase.index(stage_loop)
...
def test_gold_failure_fallback_restores_origin_main_canonical_and_head_optional_outputs(tmp_path):
    import subprocess
    from pathlib import Path

    repo_root = Path(__file__).resolve().parents[1]
    script = (repo_root / "scripts" / "ci" / "daily_engine_commit_outputs.sh").read_text()
    helper_start = script.index("GOLD_MATERIAL_PATHS=(")
    helper_end = script.index("# normalize the EXACT tree")
    ...
    origin_values = {
        "data/commodity/latest.json": "origin latest\n",
        "data/quality/china_gold_premium.json": "origin receipt\n",
        "site/commodities.html": "origin page\n",
        "data/commodity/complex_latest.json": "origin optional\n",
    }
    for name, value in origin_values.items():
        target = work / name
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(value)
    run("git", "add", ".")
    run("git", "commit", "-qm", "origin")
    origin_sha = run("git", "rev-parse", "HEAD")
    run("git", "update-ref", "refs/remotes/origin/main", origin_sha)

    head_values = {
        "data/commodity/latest.json": "candidate stale latest\n",
        "data/quality/china_gold_premium.json": "candidate stale receipt\n",
        "site/commodities.html": "candidate stale page\n",
        "data/commodity/complex_latest.json": "candidate optional\n",
        "data/commodity/cycle_positions.json": "candidate cycle\n",
    }
    ...
    for name in head_values:
        (work / name).write_text("partial failed rebuild\n")
    ...
    for name in (
        ...
        assert (work / name).read_text() == origin_values[name]
    for name in (
        "data/commodity/complex_latest.json",
        "data/commodity/cycle_positions.json",
    ):
        assert (work / name).read_text() == head_values[name]

    status = run("git", "status", "--short")
    assert "data/commodity/complex_latest.json" not in status
    assert "data/commodity/cycle_positions.json" not in status
    assert "data/commodity/latest.json" in status
    assert "data/quality/china_gold_premium.json" in status
    assert "site/commodities.html" in status


def test_gold_refresh_failure_restores_safe_outputs_after_normalization_before_stage():
    from pathlib import Path

    repo = Path(__file__).resolve().parents[1]
    text = (repo / "scripts" / "ci" / "daily_engine_commit_outputs.sh").read_text()
    post_rebase = text[text.index("git rebase --autostash -X theirs origin/main") :]
    ...
    assert "gold_post_rebase_refresh_failed=" in post_rebase
    assert post_rebase.count("gold_post_rebase_refresh_failed=1") >= 2
    assert final_fallback in post_rebase
    assert post_rebase.index(normalize) < post_rebase.index(final_fallback)
    assert post_rebase.index(final_fallback) < post_rebase.index(stage_loop)
