...
def test_c3_uses_one_existing_carrier_and_preserves_authority_ceiling() -> None:
    current = _record(WORKSTREAM)
    waves = {row["id"]: row for row in current["waves"]}
    c3 = waves["C3"]
    assert c3["status"] == "in_progress"
    assert c3["pr"] == 871
    assert "33d77c3fa94b9b8127d5a08974da9033091c6137" in c3["next_action"]
    assert "merge queue" in c3["next_action"]
    assert "BUILT_NOT_PROVEN" in c3["next_action"]
    assert "Mastermind #871 is the sole C3 implementation carrier" in " ".join(current["do_not_redo"])
...
def test_latest_handoff_names_queue_boundary_and_d1_no_effect() -> None:
    handoff = _record(HANDOFF)
    actions = " ".join(handoff["next_actions"])
    assert "3d0e96a15b541b8ed056ae5b8edd5307f0f63371" in actions
    assert "No merge or protected readback is yet proven" in actions
    assert "d79d2ec3537d8eb060055731a7c3cebee0c543eb" in actions
    assert "READ/A0" in actions
    assert any(row["claim"] == "Mastermind #871 is merged and protected." for row in handoff["unverified"])
    assert "Do not bypass the merge queue" in " ".join(handoff["do_not_redo"])
