...
gear.focus = () => {
  ...
  // Safari/Chromium may expose null here for programmatic focus; this is the
  // historical failure shape the production guard must consume.
...
// Focus restoration must not poison the next genuine external focus request.
...
def test_settings_focus_restore_executes_without_reopening_shared_nav() -> None:
    """Real shipped Settings handlers close on Escape/x and still allow fresh focus."""
    for source in (TEMPLATE_THEME_JS, SITE_THEME_JS):
        ...
        # Discrimination: reinstate the historical close(); gear.focus() shape.
        # The same runtime harness must observe the pane reopening.
