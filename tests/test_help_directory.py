...
# Frozen fixture (MO-B F13-4 R1/R2): every user-facing merge since 2026-09-06 that a
...
# Generated by: gh pr list --state merged --search 'merged:2026-09-06..<cutoff>'
# --limit 500 --json number,title,mergedAt,files | census filter (user-facing rule below)
# RULE: a PR is user-facing iff any file path starts with templates/, site/*.html,
# data/product/, or a lib/ module that renders reader copy.
_MERGES_SINCE_0906 = (
    (6926, "2026-09-08"),
    (6900, "2026-09-08"),
    (6873, "2026-09-08"),
...
def test_changelog_covers_every_user_facing_merge_since_0906() -> None:
    """Every user-facing PR in the frozen census fixture is either in the changelog
    or in the exclusion list.  A PR is user-facing iff any file path starts with
    templates/, site/*.html, data/product/, or a lib/ module that renders reader copy."""
    import json as _json

    fixture_path = ROOT / "tests" / "fixtures" / "help" / "merged_prs_2026-09-06_to_2026-09-19.json"
    ...
    user_paths = ("templates/", "site/")
    skip_dirs = (
        "agentos/", "research/", "docs/", "mockups/evidence/", "engine/",
        "scripts/", "config/", "tests/fixtures/", ".github/", "wrangler",
        "requirements", "calibration/", "collectors/", "content/",
    )
    census_prs = set()
    for pr in census:
        # gh pr list --json files returns a list of path strings
        paths = pr.get("files", [])
        user_paths_in_pr = [p for p in paths if p.startswith(user_paths)]
        if user_paths_in_pr and not any(
            p.startswith(s) for p in user_paths_in_pr for s in skip_dirs
        ):
            census_prs.add(pr["number"])

    changelog_prs = {e["pr"] for e in product_changelog(ROOT)["entries"]}
    excluded_prs = set(exclusions.keys())

    in_changelog = census_prs & changelog_prs
    excluded_correctly = census_prs & excluded_prs
    unaccounted = census_prs - changelog_prs - excluded_prs

    assert (
        not unaccounted
    ), f"unaccounted user-facing PRs (not in changelog and not in exclusions): {sorted(unaccounted)}"
    ...
    assert newest_fixture == "2026-09-13"
...
def test_product_changelog_limit_drops_old_entries() -> None:
    ...
    rows (2026-09-04 entries that are older than the oldest 2026-09-08 row above them).
    The current YAML has 22 entries; limit 20 was insufficient before 7 entries were added.
    """
    import json

    # This test does not call gh — it reads the committed census fixture.
    with open(ROOT / "tests" / "fixtures" / "help" / "merged_prs_2026-09-06_to_2026-09-19.json", encoding="utf-8") as f:
        census = json.load(f)

    all_entries = product_changelog(ROOT)["entries"]
    assert len(all_entries) >= 22, f"expected >=22 entries, got {len(all_entries)}"
    ...
    # With limit=40 they must be present
    full = product_changelog(ROOT, limit=40)
    full_prs = {e["pr"] for e in full["entries"]}
    assert 6845 in full_prs, "cycle-six (6845) must be in limit=40 result"
    assert 6836 in full_prs, "macro-suite (6836) must be in limit=40 result"
