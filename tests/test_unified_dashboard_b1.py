...
tests continue to pin the component-level binding contract against the REAL
engine contracts (R-A, R-H):
...
# --------------------------------------------------------------------------- #
# Inline-style boundary for the retained candidate.
# --------------------------------------------------------------------------- #

def test_hero_template_only_allows_named_b2w2_caveat_style_block():
    """The retained candidate may carry only its token-scoped B2W2 caveat block."""
    src = (TEMPLATES / "_unified_dashboard_hero.html.j2").read_text()
    blocks = re.findall(r"<style>(.*?)</style>", src, re.S | re.I)
    assert len(blocks) == 1, (
        "hero template may carry exactly one named B2W2 caveat style block; "
        f"found {len(blocks)}"
    )
    block = blocks[0]
    assert ".mx-spine-caveat" in block
    assert 'html[data-theme="light"] .mx-spine-caveat' in block
    assert "color-mix(" not in block
    assert not re.search(r"#[0-9a-fA-F]{3,8}|rgba?\(|hsla?\(", block), (
        "the caveat exception must remain token-scoped rather than minting raw colours"
    )
...
# Component-level engine/binding tests above remain authoritative for the held
# candidate itself; route composition is intentionally simpler.
# --------------------------------------------------------------------------- #
