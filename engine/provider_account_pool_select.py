...
def select(pool_id: str, rows: list[AccountObservation], sticky: str | None = None) -> AccountSelection:
    ...
    key = lambda row: tuple(101 if v is None else v for v in (row.rolling_used_percent, row.weekly_used_percent, row.monthly_used_percent)) + (row.account_id,)
    chosen = min(eligible, key=key)
    return AccountSelection(pool, chosen.account_id, "best_observed_headroom", False)
