...
def _run_brain_loop(
    ...
    answer_text = ""
    tool_call_count = 0
    command_turn_id = uuid.uuid4().hex[:16]
    ...
    while tool_call_count < tool_budget:
        ...
        round_batch_id = f"brain_{command_turn_id}_{tool_call_count}"
        command_seq = 0
        for block, result in zip(tool_blocks, round_results):
            ...
            if tool_name in _CHART_COMMAND_TOOLS and result.get("client_executed"):
                if result.get("v") == 2:
                    commands.append(_flat_command(result, batch_id=round_batch_id, seq=command_seq))
                    command_seq += 1
                else:
...
def _run_brain_loop_stream(
    ...
    while tool_call_count < tool_budget:
        ...
        for block, result in zip(tool_blocks, round_results):
            ...
            if tool_name in _CHART_COMMAND_TOOLS and result.get("client_executed"):
                if result.get("v") == 2:
                    wire = _flat_command(result, batch_id=round_batch_id, seq=command_seq)
                    command_seq += 1
                else:
