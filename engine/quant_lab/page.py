...
def _methods() -> list[dict]:
    ...
    for key, spec in specs_mod.METHODS.items():
        ...
        out.append({
            ...
            "display_kind": spec.get("display_kind", "measurement"),
            "proposer": spec["proposer"], "source_kind": spec["source_kind"],
            "one_line": spec["one_line"], "one_line_zh": spec.get("one_line_zh"),
            "shape": spec["shape"],
            "proposal_says": spec["proposal_says"],
            "proposal_says_zh": spec.get("proposal_says_zh"),
            "not_a_ranker_because": spec["not_a_ranker_because"],
            "not_a_ranker_because_zh": spec.get("not_a_ranker_because_zh"),
            "our_test": spec["our_test"], "our_test_zh": spec.get("our_test_zh"),
            "house_rulings": spec["house_rulings"],
            "still_live": spec["still_live"], "still_live_zh": spec.get("still_live_zh"),
            "reopen_when": spec["reopen_when"], "reopen_when_zh": spec.get("reopen_when_zh"),
