"""Deterministic longitudinal evidence deltas over two grounded institutional RIOs.
...
SCHEMA = "mastermind.research_intelligence.evidence_delta.v1"
SUMMARY_SCHEMA = "mastermind.research_intelligence.evidence_delta_summary.v1"
COMPARISON_KIND = "deterministic_rio_surface_delta"
...
def compare_institutional_rio_evidence(
    previous: Any,
    current: Any,
    *,
    topic_key: str,
    ...
    surface_change_dimensions: list[str] = []
    if direction_changed:
        surface_change_dimensions.append("thesis_direction")
    if conviction_changed:
        surface_change_dimensions.append("thesis_conviction")
    if thesis_support_changed:
        surface_change_dimensions.append("thesis_support")
    if mechanisms["added_sha256"] or mechanisms["removed_sha256"]:
        surface_change_dimensions.append("mechanisms")
    if claims["added_sha256"] or claims["removed_sha256"]:
        surface_change_dimensions.append("claims")
    if numbers["added_sha256"] or numbers["removed_sha256"]:
        surface_change_dimensions.append("numbers")
    if entities["added"] or entities["removed"]:
        surface_change_dimensions.append("entities")
    surface_change_dimensions.extend(f"category_{field}" for field in changed_categories)
    surface_change_dimensions = [
        dimension for dimension in _SURFACE_CHANGE_DIMENSIONS
        if dimension in surface_change_dimensions
    ]
    surface_change_detected = bool(surface_change_dimensions)
    ...
    return {
        ...
        "semantic_belief_authority": "none",
        ...
        "surface_change_dimensions": surface_change_dimensions,
        "surface_change_detected": surface_change_detected,
...
def summary(delta: Any) -> dict[str, Any]:
    ...
    if not isinstance(delta, dict) or delta.get("schema") != SCHEMA:
        raise ValueError("unexpected evidence delta schema")
    if delta.get("text_visibility") != "metadata_only":
        raise ValueError("evidence delta visibility is invalid")
    if delta.get("pairing_authority") != "caller_asserted_topic":
        raise ValueError("evidence delta pairing authority is invalid")
    if delta.get("comparison_kind") != COMPARISON_KIND:
        raise ValueError("evidence delta comparison kind is invalid")
    if delta.get("semantic_belief_authority") != "none":
        raise ValueError("evidence delta semantic belief authority is invalid")
    if delta.get("authority") != "descriptive_research_only":
        raise ValueError("evidence delta authority is invalid")
    ...
    if not all(isinstance(value, dict) for value in (thesis, claims, previous, current)):
        raise ValueError("evidence delta is malformed")
    ...
    if before not in allowed_directions or after not in allowed_directions:
        raise ValueError("evidence delta direction is invalid")
    ...
    if not isinstance(changed, list) or any(
        ...
        raise ValueError("evidence delta changed_categories is invalid")
    changed_categories = [field for field in _CATEGORY_FIELDS if field in changed]

    dimensions = delta.get("surface_change_dimensions")
    if not isinstance(dimensions, list) or any(
        not isinstance(dimension, str) or dimension not in _SURFACE_CHANGE_DIMENSIONS
        for dimension in dimensions
    ):
        raise ValueError("evidence delta surface_change_dimensions is invalid")
    surface_change_dimensions = [
        dimension for dimension in _SURFACE_CHANGE_DIMENSIONS if dimension in dimensions
    ]
    expected_changed_categories = [
        field
        for field in _CATEGORY_FIELDS
        if f"category_{field}" in surface_change_dimensions
    ]
    if changed_categories != expected_changed_categories:
        raise ValueError("evidence delta category dimensions disagree")
    if bool(thesis.get("direction_changed")) != (before != after):
        raise ValueError("evidence delta direction change flag disagrees")
    if type(delta.get("surface_change_detected")) is not bool:
        raise ValueError("evidence delta surface_change_detected must be boolean")
    if delta["surface_change_detected"] != bool(surface_change_dimensions):
        raise ValueError("evidence delta surface_change_detected disagrees with dimensions")
    ...
    if (
        ...
        raise ValueError("evidence delta claim counts are invalid")
    ...
    if (
        ...
        raise ValueError("evidence delta identity is incomplete")
    ...
    return {
        ...
        "changed_categories": changed_categories,
        "surface_change_dimensions": surface_change_dimensions,
        "surface_change_detected": bool(delta.get("surface_change_detected")),
        "text_visibility": "metadata_only",
        "authority": "descriptive_research_only",
    }
