...
GLOSSARY_TERMS: tuple[GlossaryTerm, ...] = (
    ...
    _t("fired-alerts-pill", "macro", "Fired Alerts", "已触发提醒",
       ...
       "Zero alerts means nothing just changed. Any red alert caps the market state at Mixed regardless of the other scores.",
       "零提醒代表没有新变化。任何红色提醒都会把市场状态压至混合。"),
    _t("macro-backdrop-matrix", "macro", "Macro Backdrop Scorecard", "宏观背景评分表",
       ...
       "Each leg explains the composite. High trend with low breadth means a narrow move — a warning sign.",
       "逐项看六项独立评分如何构成市场状态。高趋势低参与是警示。"),
    ...
    _t("posture-dial", "macro", "Posture Dial", "仓位姿态盘",
       ...
       "Answers how much risk to take, not which stocks. Pair Defensive with red, Aggressive with green.",
       "回答该承担多少风险，而非买哪只股票。防御对应红色市场，进取对应绿色。"),
    _t("sector-heat-strip", "macro", "Sector Heat Strip", "板块热力条",
       ...
       "快速列出当前哪些板块走强、哪些落后——仅作参考，不是买入信号。",
       _M, 82, "Sector Heat Strip (macro hero area)", _MACRO,
       "Context for the rotation narrative. A heating sector with weak breadth may mean the move is narrow.",
       "仅作轮动叙事参考。某板块走强但广度偏弱，行情可能偏窄。"),
    _t("leadership-rotation", "macro", "Leadership & Rotation", "领涨轮动",
       ...
       "First lane tracks where leadership moves now; second tracks whether past hardware damage has healed. Neither moves market state alone.",
       "首栏展示当下领涨轮动，次栏追踪硬件损伤是否修复。两者都不会单独改变市场状态。"),
    ...
    _t("regime-badge-us", "us-stocks", "Regime Badge", "周期徽章",
       ...
       "Use for which sectors and factors are favoured. For market turns, use the macro Risk Radar, not this badge.",
       "用于板块与因子偏好。判断市场拐点应使用宏观风险雷达，而非本徽章。"),
    _t("posture-chip-us", "us-stocks", "Posture Chip", "姿态标签",
       ...
       "Defensive or Careful means lean cautious with smaller gross. Constructive or Aggressive means scale into setups with regime support.",
       "防御或谨慎意味着降低仓位；进取或积极则按当前环境加仓。"),
    ...
    _t("insider-buy-chip", "us-stocks", "Insider Buying Chip", "内部人买入标签",
       ...
       "Insider chip with a top-tier gate pass is higher-confidence. Standalone without one, it is context only.",
       "内部人买入标签与顶级入场信号同现时信心更高。单独出现仅为参考。"),
    ...
    _t("sue-earnings-chip", "us-stocks", "Earnings Surprise Chip", "财报超预期标签",
       ...
       "Supporting context only — the measured edge here is weak, so never treat a beat alone as a buy.",
       "仅作辅助参考——该指标实测优势很弱，切勿仅凭超预期买入。"),
    _t("sector-act-now-board-us", "us-stocks", "Sector Act-Now Table", "板块即时行动表",
       ...
       "Scans many names at once. Top entry tier first, then alpha rank within tier.",
       "便于一眼扫描多只股票。先看顶级入场等级，再看等级内的相对强度排名。"),
    ...
    _t("market-state-score-china", "china", "China Market State Score", "中国市场状态分",
       ...
       "Green = trend-follow, yellow = size smaller, red = defend capital first. A present-state read, not a forecast.",
       "绿色顺势加仓，黄色缩小仓位，红色优先保护本金。是当下读数，不是预测。"),
    _t("regime-quadrant-pill-china", "china", "China Regime Quadrant", "中国周期象限",
       ...
       "Use for sectors and factors structurally favoured, not for timing. Shifts slowly; market state score is the faster read.",
       "用于增长与通胀背景下的板块偏好，非择时；旁侧状态分更快。"),
    _t("path-chart-china", "china", "Recent Trend Chart", "近期趋势图",
       ...
       "Rising toward green means improving in real time; falling into red means deteriorating. Context only, never act on one session.",
       "向绿色上升代表实时改善，向红色滑落代表恶化。仅作参考，切勿依据单日走势行动。"),
    _t("pullback-risk-radar-china", "china", "China Pullback Risk Radar", "中国回撤风险雷达",
       ...
       "Treat elevated readings seriously: size down, protect gains. Some alerts precede moves that never arrive.",
       "高位急跌概率应严肃对待：降仓。部分警报后未必真跌。"),
    _t("what-to-do-card-china", "china", "What To Do Card", "操作建议卡",
       ...
       "Sizes risk, not picks. Careful means favour quality entries with moderate gross; Constructive means lean into regime support.",
       "决定仓位大小，不涉及选股。谨慎偏好高质量入场；进取依靠环境支持加仓。"),
    _t("market-tiles-china", "china", "Market Tiles", "指数速览",
       ...
       "They show whether the main indexes are up or down today; the regime and radar reads still govern.",
       "用于查看主要指数今日涨跌方向；周期与雷达读数仍是大局判断。"),
    _t("board-track-record-china", "china", "Board Track Record", "榜单历史战绩",
       ...
       "A CI lower bound above 50% means the board has added value over random. Evidence accruing, never a guarantee.",
       "置信下限高于 50% 时，榜单相对随机已有增量价值。证据在积累，并非保证。"),
    _t("sector-rotation-act-now-china", "china", "Sector Rotation Board", "板块轮动即时表",
       ...
       "Use Buy Now and In Favour as the screener starting list. Reduce or Avoid means positions need a thesis review.",
       "以「建议买入」与「受到青睐」为筛选起点。「减仓」或「回避」意味着持仓需要重审。"),
    _t("sector-flow-velocity-china", "china", "Sector Flow Velocity", "板块资金流速",
       ...
       "Confirms or contradicts sector rotation. Decelerating inflows may mean Buy Now is pausing; accelerating means Bottoming Watch is earlier.",
       "用资金流入或流出速度印证轮动。流速减速或已近停；加速则更早。"),
    _t("china-setup-score", "china", "China Setup Score", "中国建仓评分",
       ...
       "High score means clean setup and good timing, never a guarantee. Rank the shortlist; do not override cycle state.",
       "高分代表形态干净、时机合适，绝非上涨保证。仅用于排序候选清单。"),
    ...
    _t("buy-readiness-score-cn", "china-stocks", "China Buy-Readiness Score", "中国买入准备度评分",
       ...
       "Rank the shortlist by readiness. High score means the pattern is set up, never a guarantee of a positive outcome.",
       "按距可操作买入点的远近排序。高分不代表胜率。"),
    _t("board-track-record-cn", "china-stocks", "China Board Track Record", "中国榜单历史战绩",
       ...
       "Evidence accruing. A CI lower bound above 50% means the board has added value over random. Read with median excess.",
       "证据在积累。置信下限高于 50% 意味着榜单相对随机已增加价值。"),
    _t("washout-chip", "china-stocks", "Washout Reclaim Chip", "超跌反弹标签",
       ...
       "A washout reclaim is typically a higher-quality entry than a name that drifted low; use it to break ties.",
       "急跌后收复的超跌反弹通常优于阴跌，可在同分间择优。"),
    _t("entry-timing-chips-cn", "china-stocks", "Entry Timing Chip", "入场时机标签",
       ...
       "买入表示今日买入窗口开放；等待回调表示形态有效但价格涨幅过大；持有表示已在场内。",
       _CHS, 45, "Entry Timing Chips: \"Buy Now\" / \"Wait for Pullback\" / \"Hold\"", _CN_STOCKS,
       "Buy now means the window is open: act on a confirmed entry. Wait for pullback means patience improves risk-reward.",
       "「买入」表示买入窗口开放。「等待回调」意味着耐心能改善风险收益比。"),
    _t("stage-labels-cn", "china-stocks", "Entry Stage Label", "入场阶段标签",
       ...
       "「入场」表示当前为最佳买点；「信号已过」表示近期上榜但已大幅上涨，入场窗口已大半错过。",
       _CHS, 54, "Stage Labels: \"ENTRY\" / \"RAN LATE\" (stock cards, china_stocks mode)", _CN_STOCKS,
       "Prioritize Entry-stage cards. A late card is still listed because its picture is intact, but risk-reward has deteriorated.",
       "优先选择「入场」阶段的卡片。「信号已过」形态仍完整，但风险收益已不及原信号。"),
    _t("sector-turn-boost-chip", "china-stocks", "Sector Turn Chip", "板块转折标签",
       ...
       "Adds a second confirmer: the individual setup plus a turning sector. Use it as an extra filter between similar-ranked names.",
       "带来第二重确认：个股形态加上板块转好。在排名相近的标的之间作为额外筛选依据。"),
    _t("coiled-cohort-chip", "china-stocks", "Coiled Cohort Chip", "蓄势群组标签",
       ...
       "Coiled with a top-tier signal beats the signal alone. The star variant marks cleaner setup quality.",
       "「蓄势」指超跌后走势收敛；与顶级信号同现时，整组反转更强。"),
    ...
    _t("consensus-board", "etfs", "Consensus Board", "共识榜单",
       ...
       "Breadth first. They disagree: treat as unresolved, not a weak buy.",
       "广度优先。存在分歧视为尚无定论，而非弱买入。"),
    _t("net-conviction", "etfs", "Net Conviction", "净信心",
       ...
       "追踪基金真正投入某标的的资金规模，已剔除仅因价格上涨而扩大的部分。",
       _ET, 35, "Net conviction (pp)", _ETFS,
       "A big net means several managers made real bets. Near-zero net, large gross: funds are trading against each other.",
       "净额大代表多家经理投入真实仓位；净额接近零且总额很大，代表基金之间在互相买卖。"),
    _t("per-fund-conviction", "etfs", "Per-Fund Conviction", "单只基金信心度",
       ...
       "An equal-sized add by a concentrated thematic fund beats the same add by a sector fund. Read with fund identity.",
       "单只基金对单只股票的等量加仓，要结合基金身份看，这是基础单元。"),
    _t("flow-vs-selection", "etfs", "Flow vs Selection", "资金流入与主动选股",
       ...
       "基金持仓增加有两种原因：整体主题吸引资金流入，或经理主动看好该股胜过其他标的。",
       _ET, 53, "Flow vs selection — which half of the move was investor money", _ETFS,
       "Passive fund: investor money is the meaningful half. Active fund: the manager's picks. The mismatched half is noisier.",
       "被动型：「资金流入」是有效信号。主动型：「主动选股」是有效信号。错配一半更嘈杂。"),
    _t("dollar-estimates", "etfs", "Dollar Size Estimate", "美元规模估算",
       ...
       "Dollars rank SIZE; fund counts rank BREADTH. An incomplete figure is partial: direction unknown, never a floor.",
       "美元衡量规模，基金数量衡量广度，二者回答不同问题。不完整即部分值，方向未知。"),
    _t("measurement-windows", "etfs", "Measurement Window", "观测窗口",
       ...
       "Pooled totals are fine for how much but misleading for how fast; prefer per-day figures when comparing pace.",
       "窗口按基金披露次数、不是固定天数；合计会误导速度快慢。"),
    _t("persistence-streak", "etfs", "Persistence Streak", "持续加仓天数",
       ...
       "A rising streak is a manager still building; one just flattened may be finished, and already priced in.",
       "节奏加快代表经理仍在建仓；刚走平则可能已完成，且信息多已反映在价格中。"),
    ...
    _t("stance-line", "etfs", "Stance", "操作立场",
       ...
       "Take the stance literally: Watch, don't chase is a real lagged signal, not a soft buy.",
       "「观察——不要追高」是真实的滞后信号，其入场由你决定，而非软性买入。"),
    _t("hero-verdict-etfs", "etfs", "Hero Verdict Line", "顶部结论句",
       ...
       "Board headline, not a market call. Into risk-on: tape backs accumulation. Against risk-off: buying into weakness.",
       "榜单头条不是市场判断。顺向则大盘背景支持真实买入。"),
    _t("rotation-backdrop", "etfs", "Rotation Backdrop", "轮动背景",
       ...
       "Buying with this backdrop is easier to hold; buying against it needs a longer horizon and a smaller size.",
       "顺大盘青睐方向建仓更易持有；逆势则需更小仓位。"),
    _t("fund-coverage-table", "etfs", "Fund Coverage Table", "基金覆盖情况表",
       ...
       "Zero filings means a broken feed, not an idle fund; a deep history is what makes a streak meaningful.",
       "披露为零代表数据源中断，而非该基金无操作；历史越长，连续记录才越有意义。"),
    ...
    _t("weight-trajectory-sparkline", "etfs", "Weight Trajectory Sparkline", "持仓走势迷你图",
       ...
       "Rising line under positive conviction: manager still building. Spike then flat line: position established long ago.",
       "走势图上升代表该基金持仓在增长；走平代表持仓已久。"),
    _t("forward-windows-etfs", "etfs", "Forward Outcome Ledger", "后续表现记录",
       ...
       "A record being kept, not a claim being made: it never sizes a position or ranks the board.",
       "只是持续积累的记录，并非结论：不影响仓位大小，也不参与榜单排序。"),
