...
GLOSSARY_TERMS: tuple[GlossaryTerm, ...] = (
    ...
    _t("sector-heat-strip", "macro", "Sector Heat Strip", "板块热力条",
       ...
       "Context only for the rotation narrative. A heating sector with weak breadth usually means a narrow move.",
       "仅作轮动叙事参考。某板块走强但广度偏弱，通常意味着窄幅行情。"),
    ...
    _t("market-tiles-china", "china", "Market Tiles", "指数速览",
       ...
       "Quick orientation only. Never use one day's move to override the regime or radar read.",
       "快速了解主要指数当日涨跌方向。切勿以单日走势否定周期或雷达读数。"),
    ...
    _t("washout-chip", "china-stocks", "Washout Reclaim Chip", "超跌反弹标签",
       ...
       "Washout reclaims beat names that drifted to a low. Use as a tiebreaker between similar-scored names.",
       "超跌反弹优于缓慢阴跌至低位的标的。用于在同分候选之间择优。"),
    ...
    _t("stage-labels-cn", "china-stocks", "Entry Stage Label", "入场阶段标签",
       ...
       "Prioritize Entry cards over Ran Late. Ran Late technicals are intact, but risk-reward has deteriorated.",
       "优先选择「入场」卡片。「已迟」形态仍完整，但风险收益已不及原信号。"),
    ...
    _t("consensus-board", "etfs", "Consensus Board", "共识榜单",
       ...
       "Breadth first, size second. Four funds adding beats one fund adding four times. A SPLIT row means managers disagree.",
       "广度优先于规模。四家加仓胜过一家加仓四倍。「分歧」行意味着经理观点相左。"),
    ...
    _t("flow-vs-selection", "etfs", "Flow vs Selection", "资金流入与主动选股",
       ...
       "Passive fund: FLOW is the meaningful signal. Active fund: SELECTION is the meaningful one. Mismatched half is noisier.",
    ...
    _t("measurement-windows", "etfs", "Measurement Window", "观测窗口",
       ...
       "Windows count fund filings, not days. One window covers different real time spans for different funds.",
       "窗口按披露次数而非天数计算，不同基金的同一窗口对应的实际天数也不同。"),
    _t("persistence-streak", "etfs", "Persistence Streak", "持续加仓天数",
       ...
       "A rising streak means a manager still building — what this page surfaces early. Flat means done.",
       "节奏加快代表经理仍在建仓。节奏走平则代表建仓可能已经完成。"),
    ...
    _t("rotation-backdrop", "etfs", "Rotation Backdrop", "轮动背景",
       ...
       "Supportive tape is a reason to look, not to buy — same stance as the top of a quiet board.",
       "行情支持是值得观察的理由，而非买入理由——与会员在平静榜单顶部看到的立场一致。"),
    _t("fund-coverage-table", "etfs", "Fund Coverage Table", "基金覆盖情况表",
       ...
       "Accumulation with the backdrop is easier to hold; accumulation against it needs a longer horizon and a smaller size.",
       "顺势建仓更易持有；逆势建仓则需更长持有期和更小仓位。"),
    ...
    _t("forward-windows-etfs", "etfs", "Forward Outcome Ledger", "后续表现记录",
       ...
       "Counts tell non-members how much evidence backs the board. Rotation backdrop is public so the page is judged on data.",
       "数字告诉非会员有多少证据支撑榜单。轮动背景保持公开，便于依据数据评判。"),
