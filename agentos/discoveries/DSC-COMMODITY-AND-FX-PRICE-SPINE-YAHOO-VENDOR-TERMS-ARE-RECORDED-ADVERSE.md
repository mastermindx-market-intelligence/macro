...
so_what: >-
  ...
  because they are on disk. The binding in-repo artifact is
  `config/dataset_registry.yml` `licensing: vendor_terms_personal_use`; assumption
  (this packet): the underlying Yahoo vendor terms text was not read. Any packet
  ...
  that lists them must type them `rights_blocked` with the recorded basis, never
...
verified_by: >-
  ...
  republished to a paid product; the contracts line names
...
The binding in-repo artifact for that vendor is `config/dataset_registry.yml:63`
`licensing: vendor_terms_personal_use` (same value at `:104` on the second `vendor: yahoo`
block). Assumption (this packet): the underlying Yahoo vendor terms text was not read —
no Yahoo ToS title, URL, or retrieval date is claimed. `research/MASTERMIND_DATA_CONTRACTS.md:103`
records that `data/yahoo` (with `data/stocks` and `data/baskets/ohlcv`) is yfinance-sourced
and republished to a paid product, and that the registry row is the only place that
exposure is written down. `research/IMCE_ROUND3_ARCHITECTURE_FREEZE_BY_FABLE.md:189` lists
`CANONICAL_PRICE_TAPE` as `REUSE; yfinance exposure documented in-repo`.
`docs/QUAL_DATA_COMPLIANCE.md` still contains no Yahoo/yfinance clause, and no `DEC-*`
clears the personal-use bar.

This is a durable, non-obvious fact: two paid-surface families (FX monitoring, commodity
monitoring) both sit on a recorded in-repo personal-use vendor-terms row, never cleared.
Any future packet that widens exposure of these series (new export, new paid tier, bulk
API, redistribution) must type the feeds `rights_blocked` with that recorded basis, not
as cleared and not as `unknown`/"unrecorded", and should read
