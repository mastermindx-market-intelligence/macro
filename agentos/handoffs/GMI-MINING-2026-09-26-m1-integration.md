...
unresolved:
  ...
  - "Whether a mutation run should be a standing gate for every Mining composition task, as it is for the nuclear program. Not run this session; the frozen-probe plus inline-audit combination caught four defects without it, but it is the instrument that would answer 'are these the last of the class'."
next_actions:
  - "Own PR #7950 to MERGED at head 64a0b1dd. Watcher bqgx03oau exits on concluded checks or merge; excluded reds are the spurious 'Workers Builds: macro', ci-authority, codex and merge-queue-pilot. Then verify the module on origin/main."
  - "Only then dispatch min_t03_economic_inputs ($K/ext/args_min_t03_economic_inputs.json, already retargeted to minimax/MiniMax-M3 rounds 1 after the GLM engine collapsed 3/3 on 09-24). T03 owns engine/company_intelligence/mining_issuer_profiles.py + tests/test_mining_economic_inputs.py and may not import engine/market_ontology/* (F2)."
